...
%
The sharing capabilities of SmartIO sets our solution apart from existing \gls{pcie}-based \gls{disaggregation} solutions (including Ladon~\cite{Tu2013}), as these solutions are only able to share devices in dedicated servers.
...
\begin{itemize}
    \item \Gls{dl} inserts a remote device into the local device tree of the \gls{host}~\gls{os} by using a ``\gls{shadowdev}'', as described in \cref{sec:lending}.
        ...
        %
        We describe this in further detail in \cref{nossdav,srmpds,cc,tocs}.
...
\end{itemize}
%
By making remote devices and memory resources both appear and behave as if they are locally installed, SmartIO differs from existing \gls{disaggregation} solutions based on \gls{rdma}.
%
