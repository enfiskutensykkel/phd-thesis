...
%
This sets SmartIO apart from existing \gls{pcie}-based solutions (including Ladon~\cite{Tu2013}), as these are only able to share devices in dedicated servers.
...
\begin{itemize}
    ...
    \item The \gls{mdev} extension to the \gls{kvm} makes it possible to \gls{passthrough} remote devices to a \gls{vm}, as detailed in \cref{sec:mdev}.
        %
    ...
    \item The \gls{apiext} brings device-oriented programming semantics and device driver support functions to the \gls{sisciapi}.
        ...
        %
        As the physical location of devices and \glspl{memorysegment} is abstracted away, \gls{userspace} device driver using our extension can be written as if all resources are local.
        %
        The \gls{apiext} is described in \cref{tocs}.
\end{itemize}
%
These three sharing methods set SmartIO apart from existing \gls{disaggregation} solutions.
%
Contrary to \gls{rdma}-based solutions, we do not require any \glspl{middlewareservice} and avoid any adaptations to existing software.
%
...
\objperformance*%
%
With SmartIO, it is not necessary to interact with a device driver running on the remote system, as remote resources can be mapped using the \gls{ntb} and accessed over standard \gls{pcie}.
%
...
%
To overcome this, our SmartIO implementation supports using the \gls{borrower}'s \gls{iommu} to create continuous memory ranges that can be mapped as ``\glspl{dmawindow}'' through the \gls{lender}'s \glspl{ntb} in advance of use.
%
...
%
%As described in \cref{sec:lending}, the \gls{dl} sharing method uses the \gls{shadowdev} to intercept when a device driver on the \gls{borrower} allocates \gls{dma} buffers, in order to add pages to an \gls{iommu} range.
%
...
\end{itemize}
%
Improving performance issues involving \glspl{iommu} is a candidate for future improvements, particularly implementing support for \gls{ats}~\cite{spec:ATS} which allow devices (and \gls{pcie} switch chips) to cache resolved \gls{io}~addresses.
%
