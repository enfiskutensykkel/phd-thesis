...
%
Any \gls{pcie} device may be registered with SmartIO and shared, as demonstrated by our comprehensive performance evaluation in \paperref{tocs:eval}.
%
...
\begin{itemize}
    ...
    \item Using the \gls{sisciapi}, remote \glspl{memorysegment} are mapped directly into the virtual address space of a local application.
        ...
        %
        Moreover, using our \gls{apiext}, the physical location of both devices and \glspl{memorysegment} is abstracted away.
        %
...
%
In fact, our evaluation in \paperref{tocs:eval} prove that, when conditions are similar, SmartIO allows remote resources to be used \emph{without any performance overhead} compared to using local resources.
%
...
\begin{itemize}
    ...
    \item Our performance experiments in \cref{srmpds,cc,tocs} also revealed that an \gls{iommu} in the data path can negatively affect \gls{dma} performance, as the \gls{iommu} may split large \gls{pcie} transactions into several, smaller-sized transactions.
        ...
        %
        Consequently, this may introduce limitations on scenarios where machines act as both \glspl{lender} and \glspl{borrower}, if maximizing \gls{dma} performance is a requirement.
        %
...
%
A device may be borrowed by several machines simultaneously, as devices can be borrowed either exclusively or non-exclusively.
%
...
%
No rebooting is required, and a device may be borrowed and returned many times.



The three sharing methods are able to memory map resources dynamically:
%
\begin{itemize}
    \item As we explain in \cref{nossdav,cc,tocs}, a remote device appears \gls{hotadded} to the local system when \gls{dl} inserts a \gls{shadowdev} into the local \gls{pcie} device tree.
        %
        When the device is borrowed, SmartIO configures a static \gls{dmawindow} to a local \gls{iommu} range.
        %
        The \gls{shadowdev} intercepts when a device driver on the \gls{borrower} allocates \gls{dma} buffers, in order to dynamically add memory pages to the local \gls{iommu} range.
        %
        This way, we are able to dynamically detect system memory used by a device driver running on the \gls{borrower}.
        %
    ...
    \item Using our \gls{mdev} method, explained in \cref{srmpds,cc,tocs},

\end{itemize}
%%
%%For the \gls{mdev} sharing method, SmartIO uses the \gls{lender}'s \gls{iommu} in addition to the \gls{borrower}'s, in order to map the device to the same \gls{guestphys} memory layout as the \gls{vmguest} as explained in \cref{sec:mdev}.
%
%Using \gls{dl}, the remote device appears \gls{hotadded} when the \gls{shadowdev} is added to the local \gls{pcie} device tree.
%%
%When the device is returned (or reclaimed), the \gls{shadowdev} is removed from the device tree and the device appears hot-removed.
%%
%No rebooting is required.
%
%%
%It is also possible to \gls{hotadd} and hot-remove devices to a running \gls{vm}, if the \gls{vmemulator} supports this for \gls{passthrough} devices.
%

%As described in \cref{sec:smartio-driver}, SmartIO keeps track of the location of \glspl{memorysegment} and devices, and is able to determine the shortest path in the cluster without requiring that the user knows anything about the underlying network topology.
%%
...
SmartIO also supports mapping the \glspl{bar} of \emph{other devices}, even when these devices reside in different \glspl{lendermachine}, as we explain in \cref{tocs:lending-p2p}.
%
