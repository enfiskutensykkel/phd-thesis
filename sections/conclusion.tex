...
\objdynamic*%
%
Devices registered with SmartIO can be borrowed by any machine, at any time.
%
...
Borrowing and returning devices is done dynamically while machines are running:
%
\begin{itemize}
    \item Devices can be distributed to physical \glspl{hostmachine} during run-time using \gls{dl}.
        %
        No reboot or \gls{bios} (re-)configuration is needed.
        %
        As explained in \cref{sec:lending}, devices borrowed with \gls{dl} appears to be \gls{hotadded} to the local system by using a \gls{shadowdev}.
        %
        Using the \gls{shadowdev}, we are able to detect when a device driver is loaded and allocates memory, in order to map it for the device. 
        %
        When a device is returned (or reclaimed), the \gls{shadowdev} is removed and the device appears to have been hot-removed.
        %
        We outline this in \cref{nossdav,cc,tocs}.
    ...
    \item Using our \gls{sisciapiext}, borrowing and returning devices is done explicitly through the \gls{api}.
        ...
        %
        Our extension also provides \gls{api} functions for mapping \gls{sharedsegment} for a device, as well as mapping \glspl{devicebar} for the local application process.
        %
        These \gls{api} functions are described in further detail in \paperref{tocs:api}.
\end{itemize}
%
SmartIO also supports borrowing devices from multiple \glspl{lender} and enabling \gls{p2pdma} transfers between them, as we explain in \paperref{tocs:lending-p2p}.
%
\Gls{p2p} can be enabled when borrowing devices using \gls{dl} or \gls{mdev}.
%
This is demonstrated in the various \gls{p2p} experiments presented in \paperref{cc:eval,tocs:eval}.
%
\Gls{p2p} can also be enabled by using the \gls{apiext}, by mapping \glspl{devicebar} of \emph{other devices} for the device.
%
The proof-of-concept \gls{nvme} driver experiment presented in \cref{sec:eval-nvme} demonstrates this.
