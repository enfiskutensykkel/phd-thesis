...
%
\begin{itemize}
    \item The \gls{dl} sharing method inserts a remote device into the local device tree of the \gls{host}~\gls{os} by using a shadow device, as described in \cref{sec:lending}.
        %
...
critical data path
mapping in advance
contary to rdma disaggregation, smartio removes the need to interact with a remote device driver
