...
%
Moreover, SmartIO also supports borrowing devices from multiple \glspl{lender} and enabling \gls{p2pdma} transfers between them, as we explain in \paperref{tocs:lending-p2p,tocs:mdev-p2p}.
%
\Gls{p2p} can be enabled when borrowing devices using \gls{dl} or \gls{mdev}, which is demonstrated in the various \gls{p2p} experiments presented in \paperref{cc:eval,tocs:eval}.
%
...
%
\Cref{nossdav,srmpds,cc,tocs} outline how our SmartIO framework has become gradually more dynamic and flexible with subsequent improvements and support for \gls{p2p}.
...
%
An experiment demonstrating this is presented in \paperref{tocs:eval-sriov}.
%
Devices that do not support \gls{sriov} may be \gls{disaggregated} in \emph{software} instead, using our \gls{exttosisciapi}.
%
Using the \gls{apiext}, a device may be borrowed exclusively by a single \gls{borrower}, or it can be borrowed by several machines simultaneously.
%
Our proof-of-concept \gls{nvme} driver presented in \paperref{tocs:nvme} demonstrate this, where several \glspl{borrower} share the same (non-\gls{sriov}) \gls{nvme}.
%
...
%
As such, the \gls{apiext} makes it possible to implement device drivers as part of distributed, shared-memory cluster applications. 
%
Any \gls{memorysegment} anywhere in the cluster can be mapped for the device so it may use native \gls{dma} to them, including \glspl{segment} in local \gls{ram} on the \gls{borrower}, \glspl{segment} in \gls{ram} on the \gls{lender}, or even \glspl{segment} in memory of a different cluster machine altogether.
%
It is also possible to map \gls{multicasting} \glspl{segment} for a device.
%
Furthermore, SmartIO also supports \gls{disaggregating} device memory.
%
\Glspl{devicebar} are automatically exported by SmartIO as \glspl{sharedsegment}, which can be mapped for \glspl{cpu} as well as \emph{other devices} (enabling \gls{p2p}).
%
Finally, SmartIO makes it possible to associate \glspl{memorysegment} with a device (rather than a machine in the cluster), allowing the location of \glspl{memorysegment} to be abstracted away in a similar fashion to devices.
%
...
%
The proof-of-concept \gls{nvme} driver experiment presented in \cref{sec:eval-nvme} demonstrates all of these capabilities.
