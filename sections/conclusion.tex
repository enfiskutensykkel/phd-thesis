...
%
Devices may be borrowed and returned many times, allowing systems to scale devices up or down dynamically based on immediate workload requirements.


As explained in \cref{sec:smartio-driver}, SmartIO uses device identifiers to look up \glspl{lendermachine} and which \glspl{ntb} to use.
%
By keeping track of both \glspl{memorysegment} and devices, we are able to resolve the location of resources. 
%
This allows software to be moved to any machine in the cluster while all systems are running, as SmartIO is able to locate devices and distribute them to \glspl{borrower}:
%
\begin{itemize}
    \item
    ...
    \item Using our \gls{sisciapiext}, borrowing and returning devices is done explicitly through the \gls{api}.
        ...
        %
        This way, a device driver implemented using the \gls{apiext} can be moved to any machine in the cluster.
        %
...
\end{itemize}
%
Finally, SmartIO also supports borrowing devices from multiple \glspl{lender} and enabling \gls{p2pdma} transfers between them, as we explain in \paperref{tocs:lending-p2p}.
%
...
%
\Gls{p2p} is also enabled when one or more \gls{devicebar} are mapped for another device using the \gls{sisciapiext}, as shown in \cref{sec:eval-nvme}.
%
As SmartIO keeps track of the location of resources in the cluster, it is able to determine the shortest path in the cluster network and configure the appropriate \glspl{ntb} along that path.
