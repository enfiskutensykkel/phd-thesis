...
%
As such, it is desirable to realize this solution by building a working prototype, in order to be able to explore potential opportunities and weaknesses along the way.
%
...
In order to validate the solution, it should be thoroughly tested.
%
For the most part, this is relatively straight forward: 
%
the fundamental requirement for the solution, namely that machines should be able to use the internal devices and memory in other (remote) machines as if these resources were locally installed.
%
A test can first be performed using local resources and then repeated for remote resources using our solution. 
%
If the results are the same, then the test has passed.
%
However, while simple comparison tests are sufficient for establishing whether the implementation is able to satisfactory meet the requirements, it is in some cases d




A wide range of test scenarios are possible.
from microbenchmarking the latency and throughput of specific \gls{io} operations (such as reading or writing to an \gls{nvme}, measuring packets  or copying memory onto a \gls{gpu})

possible to use a wide range of applications and test scenarios




wide range of tests to showcase 



, it should be tested with both functionality tests and performance tests.
%

with both functionality tests and performance tests.
Functional tests are aimed at demonstrating that these resources \emph{behave} as if they are locally installed, 


while performance tests are intended to evaluate the \emph{performance} of using remote resources.
%
%
(compared to local resources).
%
%
This includes tests using several kinds of \gls{pcie} devices and device driver implementations, multiple versions of the Linux kernel, and a wide variety of application software that rely on device \gls{io}.
%
Performance tests 

include scenarios with multiple versions of the Linux kernel, different types of \gls{pcie} devices driver implementations, several types of \gls{pcie} devices, and multiple \gls{io}



microbenchmarks, were we focus on the latency and throughput of \gls{io} operations (such as reading from a storage device or copying memory onto a GPU)

performance
how the network topology affects performance


in a performance-optimal 


compare local and remote

%for making it easier to share \gls{io} resources efficiently among machines in a \gls{pcie} cluster as our prototype.
%
...
%
%%micro bencmarks and larg-scale tests w/ realistic workloads
...
%
%%iterate these steps if tests reveal that latest version of system does not satisfactory meet the requirements
%%
%%These steps should be iterated, for example if testing reveals that the latest version of the prototype does not meet the requirements in a satisfactory manner.
%%
%
...
%
%
%%As stated in \cref{sec:objectives}, the goal of this dissertation is to build a system for sharing and distributing \gls{io} resources in a \gls{pcie}-networked cluster, which we can then test and evaluate according to a set of requirements 
%%%
%%If our implementation does not meet these 
