...
%
It is generally not possible to predict in advance which memory addresses a device driver may ues, yet \gls{ntb} mappings must be in place before the driver for example initiates \gls{dma} transfers.
%
Deferring setting up these mappings until the device driver initiates \gls{dma} is not realistic, as this would introduce communication overhead in the critical path when synchronizing with the remote system. 
%
A mechanism for preparing the necessary memory-mappings through the \gls{ntb} in advance is needed.



The naive solution is to map the entire system memory of any machine using a device for the device.
%
However, this workaround does not scale very well as the memory requirements increase for each machine added to the cluster.
%
Using dedicated servers able to support these memory requirements to host devices would put a limitation on the cluster network topology.
%
To avoid that machines become potential bottlenecks, centralized servers must be avoided, and distributed, peer-to-peer sharing must be supported instead. 
%
It must be possible for \emph{all} machines in the cluster to share their own internal resources, and all machines must be able to access these shared (remote) resources, regardless of the cluster network topology.
%
This requires a mechanism for sparingly mapping remote resources, such as memory or device registers, on demand, without requiring that systems are aware of the underlying \gls{pcie} topology.
