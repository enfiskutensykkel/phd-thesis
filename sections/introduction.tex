...
%
As many solutions for distributed \gls{io} already exist, the motivation for this dissertation is not simply to make it easier for machines to share \gls{io} resources efficiently, but to attempt to do so using an altogether new way that makes it possible to unify traditional device \gls{io} with distributed, shared-memory computing.
%
...
%
Instead, we have followed the technocratic paradigm by iteratively designing, implementing, and testing a solution according to the requirements and specifications given in \cref{sec:problem}.



To validate our solution, we have performed both functional tests and performance tests.
%
Both types of tests serve the same purpose, namely to confirm that our solution makes it possible for machines to access internal devices and memory in other (remote) machines, as if these resources were local to the machines using them.
%
Functional tests are aimed at demonstrating that these resources \emph{behave} as if they are locally installed, while performance tests are intended to evaluate the \emph{performance} of using remote resources (compared to local resources).
%
%
