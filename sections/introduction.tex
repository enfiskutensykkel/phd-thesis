...
%
The scope of this dissertation is the implementation and performance evaluation of the rudimentary mechanisms that make this framework possible: \textbf{the discovery, addressing, access, and use of remote \gls{pcie}-attached resources}.


Additional research topics emerge from enabling this kind of sharing functionality;~e.g., how should resources be provisioned in order to achieve the best trade-off between fairness and performance, or how should workloads be scheduled on cluster machines in order to optimize resource utilization.
%
The security implications of allowing remote machines to use and control internal system resources also become relevant in the context of enabling direct access.
%
A ``finished product'' should take all of this into consideration, and ideally include tools and services for managing resources and orchestrating workloads.
%
However, we consider this to be outside the scope of this dissertation.
...
%
Memory mappings between machines can be set up and torn down using Dolphin's existing \gls{ntb} driver, and application software may use the \gls{sisci} programming \gls{api} to interact with this low-level driver in order to implement shared-memory communication.
%
In order to fit into this existing cluster networking foundation, we will implement our sharing framework as part of their driver stack.
%
This makes it possible to extend the existing \gls{sisci}~\gls{api} with device-oriented semantics and device driver support functions, rather than requiring the development of an entirely new \gls{api} for the purpose of supporting our sharing and \gls{disaggregation} concepts.
%
...
One of the main objectives of this dissertation is to support sharing of standard \gls{pcie} devices without requiring any modifications to existing device driver implementations or application software~(\cref{obj:transparent}).
%
Standard pcie devices
As the main focus of this dissertation is 
%a limitation because device drivers are written in this way
...
As outlined in \cref{sec:motivation}, one of the central motivations of our framework is that both \gls{host}~machines and \glspl{vm} should be able to use shared devices.
%
...
%
While many different \glspl{os} exist, this intercepting mechanism involves intricate manipulation of \gls{os} and device driver interfaces.
%
Because of this, we have chosen to implement our solution for the Linux kernel, as it is open source and its internals can be studied from easily available source code.
%
Additionally, although several \gls{vm} \gls{hypervisor} implementations exist for Linux, we have decided to extend the \gls{kvm} in order to support sharing resources with \gls{vm} \glspl{guest}, for the same reason.
%
Regardless, we make a point of making our work as general as possible, so that it is possible to support several \emph{versions} of the Linux kernel and use our implementation on different Linux distributions.
