...
The subject of this dissertation touches on several sub-areas of computer~science, including computer and hardware architecture, distributed computing, \gls{os} fundamentals, and communication networks.
%
...
%
As many solutions for distributed \gls{io} already exist, the motivation for this dissertation is not simply to make it easier for machines to share resources efficiently, but to attempt to do so using an altogether new way that makes it possible to unify traditional device \gls{io} with distributed, shared-memory computing.
%
As such, it was desirable to realize this solution by \textbf{building a working prototype}, in order to be able to explore potential opportunities and weaknesses along the way.
%
On this basis, neither the rationalist nor the scientific paradigm are particularly well-suited.
%
...
%
Instead, we follow the technocratic paradigm by iteratively designing, implementing, and testing a solution according to the requirements and specifications given in \cref{sec:problem}. 
%
The different iterations and gradual improvements of our solution can be seen in \crefrange{paper:nossdav}{paper:tocs}.


To provide a complete evaluation, the solution must be \textbf{thoroughly tested}:
%
\begin{itemize}
    \item %\textbf{Functionality testing}: 
        A wide range of functionality tests for should be included to prove that the solution is general and does not require any special adaptions.
        %
        As we make a point of using unmodified hardware and software, commodity devices and widely available software, such as standard benchmarking tools and well-known applications, should be used in tests.
        %
        Furthermore, tests should be performed using several different types devices and device driver implementations.
        %
        The evaluation should also contain a variety of different cluster network topologies and machine configurations, including \glspl{vm}.
        %
        Ideally, the evaluation should showcase several sharing scenarios that are made possible by our solution.

    \item %\textbf{Comparison testing}: 
        The performance of the critical \gls{io} data path should be examined thoroughly through a wide range of latency and throughput benchmarks.
        %
        Since the solution allow machines to use the internal devices and memory in other (remote) machines as if these resources were locally installed, we can use comparison testing.
        %
        ...
        %
        Then, the tests can repeated for remote resources using our solution, allowing us to compare the results and pinpoint any performance differences.
        %
        If there are any differences, these tests help us identify which component of the implementation needs to be improved.
        %
        We can also use this kind of testing to investigate how different cluster network topologies affect the data path, particularly when resources from different physical machines across the cluster are involved.
    ...
    \item %\textbf{Stress testing}: 
        ...
        %
        By running a real-world application using several \gls{io} resources, we can observe the accumulated effects of any performance overheads caused by the implementation that are not visible on their own.
        %
    ...
    \item %\textbf{Exploratory testing \& experimentation}: 
        Part of the evaluation should be to identify and explore new possibilities that are enabled by the solution.
        %
        For example, as CUDA unified memory~\cite{url:unified-memory} can be supported even for \glspl{gpu} that reside in different machines, we should perform experiments with direct memory-to-memory transfers.
        %
        We should also explore other opportunities that are enabled by the solution, such as using memory \gls{disaggregation} to exploit memory locality optimizations or using \gls{pcie} \gls{multicasting} to replicate data across several machines in a single operation.
        %
        Performing these tests have the added benefit of showcasing capabilities enabled by our solution that other resource sharing solutions lack.

\end{itemize}
%
For the sake of convenience, functionality testing is implicit in the experiment results presented in this dissertation.
%
Also, in an attempt to reduce the test matrix, the presented experiments use primarily Intel~Xeon \glspl{cpu} and \glspl{nvme}, \glspl{nic}, and Nvidia \glspl{gpu}.
%
However, it should be noted that during the development of the solution, many additional devices were used in testing, such as \glspl{fpga}, sound cards, several \glspl{gpu} from both AMD and Intel, and \gls{pcie}-attached cameras. 
%
Several \gls{cpu} architectures were also used during development, such as \glspl{cpu} from AMD and other Intel \glspl{cpu}.

%Our solution is to use the inherent memory-mapping capabilities of \glspl{ntb} to develop a mechanism for making resources appear and behave exactly as if they were part of the local device tree, thus eliminating any distinction between remote and local 
%solution for sharing devices and memory in a way that seamlessly unifies traditional distributed \gls{io} and distributed, shared-memory computing.
%transparent to even device drivers
