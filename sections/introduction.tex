...
The scope of this dissertation is to develop a solution for sharing standard \gls{pcie}-attached resources using \glspl{ntb}.

The work of this dissertation is part of a research project that has been performed in collaboration with Dolphin Interconnect Solutions~(Dolphin).\footnote{RCN/NFR \#235530: \emph{Unified PCI Express for Distributed Component Virtualization}}
%
As such 


%This dissertation is performed mainly in the context of the \emph{Unified PCI Express for Distributed Component Virtualization} project (Norwegian research council 

done in context of pcie project number so and so, natural to use dolphin ntbs %ikke prøvd på noe annet enn
\begin{itemize}
    \item use standard PCIe, no change to standards
    \item outside scope: orchestration, algorithms for sharing, fairness, etc... not a finished product
    \item also outside scope: safety, security, encryption
    \item dolphin ntbs
    \item commodity servers, multi platform amd, intel, arm, no supercomputing (not 50,000 nodes)
    \item for linux only?A
    \item while we have implemented for dolphin, the ideas are general
\end{itemize}
