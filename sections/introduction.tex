...
Sharing several kinds of devices, for example both \glspl{gpu} and \glspl{nvme}, not only requires different \gls{disaggregation} implementations, but integrating these distinct implementations with each other may also be a challenge.




%There is a need for a flexible yet simple-to-use solution for sharing resources and enabling distributed \gls{io} among networked machines, 
...
%
Using such \gls{ntb}-capable switch chips to implement peripheral devices, independent computer systems can interconnect with plug-in adapter cards and external cables~\cite{Ravindran2008,Shim2018,Lim2019,Tu2014,Tu2018}, as depicted in \cref{fig:cluster-example}.
...
%
As illustrated in Figure~\ref{fig:direct-access}, software overhead can be avoided, since all memory address translations can be done in \gls{ntb} hardware.
%
