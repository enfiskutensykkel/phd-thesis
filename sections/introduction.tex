...
Several related research topics emerge from enabling this envisioned sharing framework.
%
...
%
In order to fit our work into this existing cluster networking foundation, we rely on Dolphin's \gls{ntb} hardware for our implementation, and have integrated our sharing framework into their driver stack.
%
...
%
To facilitate software \gls{disaggregation}, for example, we can add device-oriented semantics and device driver support functions to the already existing shared-memory functionality provided by \gls{sisci}, instead of requiring that application software must convert and translate and between different \glspl{api}.
%
Even so, it should be pointed out that while our specific implementation is using Dolphin \glspl{ntb}, the ideas and concepts of our work are general, and can in fact be implemented for \emph{any} \gls{ntb} hardware that has similar capabilities.
...
As per \cref{obj:distributed}, sharing of any \gls{pcie} device is supported, from \gls{pcie}~1.0 devices up to \gls{pcie}~4.0 devices.
%
Consequently, validating that the sharing framework is able to support a wide range of common (and commodity) devices, such as \glspl{nvme}, Ethernet~\glspl{nic}, and \glspl{gpu}, are covered by this dissertation.
%
...
%
Since devices can therefore only be used by one machine at the time, a limitation of the evaluation of our implementation is that it includes scenarios with cluster networks of only a few machines,
%
However, for evaluating resource \gls{disaggregation} in software, where a single device may be used by more than one machine simultaneously, more advanced network topologies and larger clusters (of up to 60~machines) are used.
...
%
%evolution of SmartIO seen in papers
%something something iterations senn in papers
