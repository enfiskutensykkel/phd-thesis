...
In order to meet the latency and throughput requirements of data-driven and compute-heavy workloads, there is a need for flexible, yet efficient, sharing of \gls{io} resources in computing clusters.
%
...
%
Using our solution, \gls{io} resources are no longer locked to individual machines, and can instead be shared freely with other machines in the cluster.
%
...
As per our objectives, sharing of any \gls{pcie} device should be supported, from \gls{pcie}~1.0 devices up to \gls{pcie}~4.0 devices.
%
Our evaluation includes a wide range of common (and commodity) devices, like \glspl{nvme}, Ethernet~\glspl{nic}, and \glspl{gpu}.
%
However, the framework should also not require any modifications to existing device driver implementations or device hardware.
%
...
\begin{description}
    \item[\cref{chapter:smartio}]
        gives a summary of SmartIO.
        %
        We describe the initial idea and challenges for SmartIO, and provide an overview of the implementation.
        %
