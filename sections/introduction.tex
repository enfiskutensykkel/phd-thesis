...
Extending the \gls{pcie} bus out of a single computer and using it as a high-speed interconnection technology is a compelling alternative to distributed \gls{io} over traditional networking technologies.
%
...
%
However, as \gls{pcie} was originally designed to connect devices to the local \gls{cpu} on a motherboard, individual computer systems operate with different \gls{pcie} address domains.
%
State of the art \gls{pcie}-based disaggregation solutions solve this by reling on virtualization support in \gls{pcie} switch chip hardware to dynamically partition a shared \gls{pcie} interconnection fabric~\cite{Chung2018,whitepaper:Microsemi,url:rackscale,url:liqid,url:gigaio}. 
%
Rather than being installed into individual machines, devices are instead attached directly to a so-called ``top-of-rack'' switch.
%
By creating a logical device tree for each individual machine that maps to the machines' respective address domains, devices may be logically assigned and reassigned to different machines on the fly.
%
However, this partitioning of the \gls{pcie} fabric into separate, logical address domains means that it is not possible to support native host-to-host communication between machines over \gls{pcie}.
%
As such, \gls{pcie}-based \gls{disaggregation} solutions stop short of enabling true \gls{pcie} networking, and distributed applications running on these machines must instead use other networking technologies to communicate, such as Ethernet or InfiniBand.
%
\lgls{disaggregation}{Disaggregating} a device and sharing it with multiple machines requires either virtualization support in the device itself,~i.e., \gls{sriov}, or additional distribution methods like \gls{rdma}.
...
%
Since all memory address translations are done in the \gls{ntb} hardware, very low latency memory-to-memory communication is supported~\cite{Lim2019}.



More interestingly, however, is the fact that in such \gls{ntb}-based networks, all \glspl{cpu} and internal \gls{pcie} devices are same, shared \gls{pcie} fabric.
%
