...
Extending the \gls{pcie} bus out of a single computer and using it as a high-speed interconnection technology is a compelling alternative to traditional networking technologies~\cite{Ravindran2008}.
%
As \gls{pcie} is the standard for connecting \gls{io} devices to a local computer system~\cite{Fountain2005}, only using native \gls{pcie} would have clear performance advantage over distributed \gls{io} using traditional networking technologies, since conversion between network protocol and \gls{io} bus would not be necessary.
%
Notwithstanding, as \gls{pcie} was designed as a local \gls{io} bus standard, connecting devices to the \gls{cpu} on a motherboard, individual computer systems operate with independent \gls{pcie} address domains.
%
...
%
Rather than being installed into individual machines, devices are instead attached directly to one or more so-called ``top-of-rack'' switches.
%
...
%
However, by partitioning the fabric into separate, logical address domains, these \gls{pcie}-based \gls{disaggregation} solutions do not support communication between machines over \gls{pcie}.
%
As such, they stop short of enabling true \gls{pcie} networking, and software processes running on these machines must use other networking technologies in order to communicate with each other.
...
Due to its intrinsic memory addressing abilities, it is desirable to use \gls{pcie} as the enabling networking technology for distributed, shared-memory applications~\cite{Shim2018,Lim2019}.
%
...
%
Using such \gls{ntb}-capable switch chips to implement peripheral devices and cluster switches, it becomes possible to connect independent computer systems with plug-in host adapter cards and external cables, as illustrated in \cref{fig:cluster-example}.
%
...
%
Since all memory address translations are done in the \gls{ntb} hardware, very low latency communication is supported~\cite{whitepaper:Regula2004,url:Meduri2011}.
%
More interestingly, however, is the fact that in such networks, all \glspl{cpu} and internal \gls{pcie} devices are same, shared \gls{pcie} fabric.
...
%
not only would this require extensive modifications to existing device driver software in order to manage multiple address space layouts, which in itself is infeasible due to the vast amount of devices and device drivers that exist, but the programming complexity would also greatly increase when the number of machines in a cluster increase beyond a few nodes.
