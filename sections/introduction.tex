...
%
Heterogeneous \gls{pcie} clusters of anywhere from 2 up to 60~nodes are supported by using \gls{ntb} adapter cards and cluster switches from Dolphin.
%
Memory mappings between machines can be set up and torn down on demand using Dolphin's \gls{ntb} driver, and application software may use the \gls{sisci} programming \gls{api} to interact with this low-level driver in order to implement shared-memory communication.


The goal of this project is to develop a new framework for sharing devices and memory resources among computer systems connected with Dolphin's \gls{pcie} networking technology.
%
...
%
As outlined in \cref{sec:problem}, the objectives as well as the shared-memory nature of this framework necessitate the use of \glspl{ntb}.
%
Therefore, we have not explored any possibilities for resource sharing using alternative networking technologies in our work, and instead rely on Dolphin’s \gls{ntb} hardware as the enabling networking technology.
%
Moreover, we have also integrated our software implementation into Dolphin's driver, in order to fit the existing shared-memory semantics provided by their software stack.
%
However, it should be pointed out that the concepts and ideas presented in this dissertation are general, and can be implemented \emph{any} \gls{ntb} hardware that supports similar capabilities.



The sharing framework should not require any modifications to existing application software or device drivers.
%
Additionally, sharing should be possible for both \gls{host}~machines and \glspl{vm}.
%
...
%
While many \glspl{os} exist, this intercepting mechanism involves intricate manipulation of \gls{os} and device driver interfaces.
%
Because of this, we have chosen to implement our solution for the Linux kernel, as it is open source and its internals can be studied.
%
Additionally, while several \gls{vm} \gls{hypervisor} implementations exist for Linux, we have decided to extend the \gls{kvm} in order to support sharing resources with \gls{vm} \glspl{guest}, for the same reason.
%
Regardless, the ideas and concepts for the sharing framework are general enough to be supported for several \emph{versions} of the Linux kernel, making it possible to use on different Linux distributions.



%performance requirement = local access
%therefore comparison tests
%therefore loads of small clusters in evaluation
%also cluster sizes, and because the nature of the mechanisms focused on small clusters 

% point about standard pcie devices (from 1.0 to 4.0) and not foreseeing special devices?


Finally, several additional research questions emerge from enabling the kind of sharing capabilities this dissertation aims for. Examples of these include how resources should be provisioned in order to achieve the best trade-off between fairness and performance, or how workloads should be scheduled on cluster machines in order to optimize resource utilization.
%
The security implications of allowing remote machines to directly use and control internal system resources is also an important topic.
%
While a ``finished product'' should take these topics into consideration, and also (ideally) include tools and services for managing resources and orchestrating workloads, we consider this to be outside the scope of this dissertation.
...
An almost identical classification of paradigms is given by the ACM Task Force on the Core of Computer Science~\cite{Comer1989}.\footnote{ACM uses the names ``theory'', ``design'', and ``abstraction'' for these paradigms instead.}
%
