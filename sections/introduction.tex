...
\begin{itemize}
    ...
    \item The scientific paradigm, which defines computer~science as a natural (empirical) science.
        ...
        If a test fails, the implementation must be revised. If an experiment ``fails'', the theory instead must be revised.
\end{itemize}
%
A similar classification of paradigms is supported by the ACM Task Force on the Core of Computer Science~\cite{Comer1989}. 
%
They additionally note that computer~science---with its own theory, mathematical analysis, experimental method, and engineering practices---is so deeply rooted into both mathematics and engineering, that not any one of the paradigms can be said to be fundamental to the discipline.



The subject of this dissertation touches on several subareas of computer~science, including computer and hardware architecture, distributed computing, \gls{os} fundamentals, and communication networks.
%
While all three paradigms can be applied to these subareas, the central research~question of this dissertation~(\cref{sec:objectives}) concern computer systems and \gls{io} resources that exist in the real world.
%
...
%The technocratic paradigm can be outlined as 
%As stated in \cref{sec:objectives}, the goal of this dissertation is to build a system for sharing and distributing \gls{io} resources in a \gls{pcie}-networked cluster, which we can then test and evaluate according to a set of requirements (\crefrange{obj:distributed}{obj:experiments}).
%%
