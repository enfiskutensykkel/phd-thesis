...
%
In theory, this would allow \emph{all} machines in the cluster to share \emph{all} of their resources, including internal \gls{pcie} devices and system memory. 
%
By avoiding centralized servers, the risk of individual machines becoming potential performance bottlenecks is reduced.
...
%
Therefore, another mechanism for using \glspl{ntb} while remaining agnostic about the address space in remote systems is needed.
%
...
%
Deferring setting up these mappings until the device driver initiates \gls{dma} is not realistic, as synchronizing with the remote system would introduce communication overhead in the performance critical path.
%
The naive solution is to map the entire system memory of all other machines in the cluster, but this workaround does not scale very well, however, as the memory requirements increases for every machine added to the cluster.
%
%In addition to an address space abstraction mechanism, 
A solution for preparing the necessary memory-mappings through the \gls{ntb}, without adding additional latency caused by communication, is also needed.
