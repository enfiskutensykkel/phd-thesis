...
%
A mechanism for preparing the necessary memory-mappings through the \gls{ntb} advance of use is needed.



Without relying on preconfiguration or requiring that systems are otherwise aware of the underlying \gls{pcie} topology, the naive solution is to map the entire system memory of any machine using a device for the device.
%
...
%
While using dedicated servers able to support these memory requirements to host devices would be possible, this would impose restrictions on the cluster network design.
%
Additionally, centralized servers should be avoided in order to limit the risk of individual machines becoming potential bottlenecks.
%
By allowing all machines in the cluster to share their own internal resources, and making it possible for all machines to use these shared (remote) resources, we would instead enable a distributed, peer-to-peer sharing model without limiting the possible cluster topologies.
%
This requires a mechanism for mapping remote resources, such as memory and device registers, both sparingly and dynamically.
