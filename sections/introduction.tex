...
%
However, since not all devices implement \gls{sriov}, the solution should also provide a device driver \gls{api} that will make it possible to \glslink{disaggregation}{disaggregate} memory resources in software.
%
This \gls{api} should provide facilities for memory-mapping device registers as well as mapping shared memory segments for a \gls{dma}-capable device.
%
...
All parts should be thoroughly tested with latency and throughput in mind, in order to reveal any potential performance bottlenecks. The solution should provide \emph{native} \gls{pcie} performance, as stated in \cref{obj:performance}.
An additional point is to use standardized test suites as far as possible, to demonstrate that application software really can be unmodified (as stated in \cref{obj:transparent}).
Moreover, a real-world workload relying on a combination of several devices, including computing accelerators, should be used to show that a solution is a viable approach for resource scaling.
Finally, a prototype device driver using the device-oriented \gls{api} (\cref{obj:api}) should be developed and evaluated. This driver should demonstrate that it is possible to \glslink{disaggregation}{disaggregate} a non-\gls{sriov} device in software, and shared with multiple machines simultaneously. The driver should also demonstrate how it can rely on memory \gls{disaggregation} capabilities to implement data path optimizations.
