...
%
If such devices are scarcely distributed in the cluster, cluster machines with \gls{io} resources may become bottlenecks, for example when a workload requires heavy computation on \glspl{gpu} or fast access to storage.
%
...
%
Being able to share and partition devices between machines in a cluster in run-time leads to more efficient utilization, as individual machines may dynamically scale up or down \gls{io} resources based on current workload requirements (\cref{fig:device-pool}).
...
There is a need for a solution for sharing resources among networked machines in a cluster that not only makes it possible to share \emph{all} types of \gls{io} devices, but should also unify the 

in a unified manner, 

but also makes accessing remote resources indistinguishable from using local resources in order

with minimal performance overhead.

shared resources indistinguishable with local resources

generic
enables sharing at different abstraction levels: 
(I)~distributing devices to physical machines in the cluster, (II)~distributing devices to \glspl{vm} running on different host~machines in the cluster, and (III)~facilitating \gls{disaggregation} of devices and memory resources in software.
%
This dissertation contributes to this goal by presenting a system for sharing and distributing \gls{io} devices among machines in a shared-memory cluster interconnected with \gls{pcie}.

In order to increase the \gls{io} resource utilization in a computing cluster, 
%
Furthermore, to meet the latency and throughput requirements of data-driven cluster computing applications, the solution should support distributed \gls{io} with minimal performance overhead and without forgoing flexibility.



%that does not forgo the performance needed to meet latency and throughput requirements of data-driven and compute-heavy workloads.

%
%There is a need for a more fundamental approach to sharing resources between machines in a cluster 
%solution that enables sharing on a more fundamental level than \gls{rdma} \gls{disaggregation} and virtualization is needed.
%%
%Distributed \gls{io} must be enabled
%that enable sharing on multiple abstraction levels: distributing devices to physical machines in the cluster, distributing devices to \glspl{vm} running on host~machines in the cluster, and providing \gls{disaggregation} capabilities 
%In order to meet the latency and throughput requirements
%To increase \gls{io} resource utilization in a computing cluster, there is a need for a general solution that enables dynamic scaling and cost-effective resource sharing between networked machines.
%%Our solution facilitates distributed \gls{io} without introducing needless performance overheads in order to meet the latency and throughput requirements of data-driven and compute-heavy workloads in cluster computing applications.
