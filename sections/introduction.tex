...
%
Moreover, by using \glspl{ntb} rather than dedicated servers or requiring that devices are attached to switches, we could enable a distributed, peer-to-peer sharing system.
%
This would allow \emph{all} machines in the cluster to share \emph{all} of their resources, including internal \gls{pcie} devices and system memory. 
%
By avoiding centralized servers, the risk of individual machines becoming potential bottlenecks is reduced.
...
%
Even though \gls{passthrough} allows the device to be used directly by a device driver running in a \gls{guest}, requiring that compute tasks run in \glspl{vm} is a constraint that may add additional system load, as \gls{cpu} cycles are spent on hosting the virtualized environment.
%
Another mechanism for using \glspl{ntb} while remaining agnostic about the address space in remote systems is needed.
%
The physical location of a resource, as well as the address space layout in the machine it is installed in, must be abstracted away without requiring \glspl{vm} (but \glspl{vm} should \emph{also} be supported).
...
%
The naive solution is to map the entire system memory of all other machines, however, this workaround does not scale very well as the memory requirements increase for each machine added to the cluster.
%
A mechanism for preparing the necessary memory-mappings through the \gls{ntb} without adding latency in the performance critical path, is needed.
