...
\item Development of a prototype \gls{nvme} device driver using our device-oriented \gls{api} (see \cref{paper:tocs}).\footnote{The prototype \gls{nvme} device driver is open source and can be found at \\\texttt{\url{https://github.com/enfiskutensykkel/ssd-gpu-dma}}}
...
\begin{itemize}
	\item \textbf{\cref{chapter:smartio}:} presents the overall ideas and challenges for SmartIO. 
		We give a high-level overview of the implementation and 
		also related work

	\item \textbf{\cref{chapter:concl}:} 

	\item \textbf{\cref{paper:nossdav}:}

	\item \textbf{\cref{paper:mmsys}:}

	\item \textbf{\cref{paper:srmpds}:}

	\item \textbf{\cref{paper:cc}:}

	\item \textbf{\cref{paper:tocs}:}
\end{itemize}
