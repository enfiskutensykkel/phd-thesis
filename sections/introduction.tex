...
\begin{figure}
    ...
    \begin{subfigure}{\linewidth}
        \centering
        \includegraphics[width=.95\linewidth]{fabric-partitioning}
        \caption{Partitioning}
    \end{subfigure}
    \par\vspace{5mm}
    \begin{subfigure}{\linewidth}
        \centering
        \includegraphics{fabric-partitioning-device-tree}
        \caption{Logical device tree} % each machine is "isolated", traditional networking is required for communication
    \end{subfigure}
    \caption[Some switch chips support partitioning the \glsxtrshort{pcie} fabric]{Partitioning}
...
%
State of the art \gls{pcie}-based disaggregation solutions solve this by relying on virtualization support in \gls{pcie} switch chip hardware to dynamically partition a shared \gls{pcie} interconnection fabric~\cite{Chung2018,whitepaper:IDT,whitepaper:Microsemi,url:rackscale,url:liqid,url:gigaio}. 
%
...
%
By creating a logical device tree for each individual machine that maps to the machines' respective address domains, switch-attached devices may be logically assigned and reassigned to different machines on the fly.
%
However, this partitioning of the \gls{pcie} fabric into separate, logical address domains does not make it possible for machines to share their \emph{internal} resources, such as memory.
%
The lack of shared memory capabilities means that is not possible to support native host-to-host communication over native \gls{pcie}.
%
Instead, other networking technologies must be used for communication, such as Ethernet or InfiniBand.
%
\lgls{disaggregation}{Disaggregating} a device and sharing it with multiple machines at the same time requires either alternative methods like \gls{rdma}, or additional virtualization support in the device itself,~i.e., \gls{sriov}.
...
\begin{figure}[!h]
	\centering
    \includegraphics[width=.95\linewidth]{cluster-example}
...
While state of the art \gls{pcie}-based \gls{disaggregation} solutions can be said to enable a composable \gls{io} infrastructure, these solutions stop short of providing any \emph{networking} capabilities over \gls{pcie}.
%
