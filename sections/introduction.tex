...
%
For enabling software \gls{disaggregation}, for example, we can add device-oriented semantics and device driver support functions to the already existing \gls{sisci}~\gls{api}. 
...
%
Our evaluation includes a wide range of common (and commodity) devices, such as \glspl{nvme}, Ethernet~\glspl{nic}, and \glspl{gpu}.
%
...
%
Nevertheless, more advanced network topologies and larger clusters (of up to 60~machines) are used in instances where a single device may be used by more than one machine simultaneously. 
%
Such instances are evaluation scenarios involving devices with multiple device functions,~i.e., \gls{sriov}, and experiments with software \gls{disaggregation} of devices and memory resources.
