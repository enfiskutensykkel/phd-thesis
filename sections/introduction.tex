...
Another challenge is the networking technology itself. Despite having been a research topic for decades, moving data to remote units over a network remains a costly operation that introduces large performance overheads, compared to using local resources.
%
As such, many network adapters support zero-copy of application memory from one system to another through \gls{rdma}~\cite{Huang2012}.
%
...
%
For example, \glspl{nvme} may be \glslink{disaggregation}{disaggregated} and shared in the cluster with very low latency using \gls{rdma}.
This is the case for \gls{nvmeof}~\cite{Guz2018}, where \gls{rdma} is used to provide direct access and avoid going through the block-layer on the \gls{os} on the server.
%
...
%
The benefit of \gls{rdma} \gls{disaggregation} is that resources may be shared by several hosts in the cluster.
%
...
%
Moreover, since \gls{rdma} \gls{disaggregation} is usually implemented as application-specific \gls{middleware}, it often requires application software to use specific programming models and semantics like message-passing.
