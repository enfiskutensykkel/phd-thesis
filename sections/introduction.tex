...
\begin{itemize}
    ...
    \item Testing and evaluation of the Device~Lending mechanism for distributing \gls{pcie} devices to remote systems (see \cref{paper:nossdav} and \cref{paper:mmsys}). Using Device~Lending, \glspl{nvme}, \glspl{gpu}, network adapters, and any \gls{pcie} device may be assigned to a remote system, appearing to the remote system as if the device has been dynamically hot-added to the system.
    ...
    Although the Device~Lending component of SmartIO makes it possible to use existing device drivers, most device drivers are written in a way that assumes exclusive control over the device. Therefore, using Device~Lending or the \gls{mdev} extension, a device (function) may only be used by a single user at the time.
    ...
    \item A comprehensive performance evaluation covering all parts of SmartIO and the implementation of performance optimizations (see \cref{paper:srmpds}, \cref{paper:cc}, and \cref{paper:tocs}). With the goal of not incurring any performance overhead beyond that of native \gls{pcie}, the performance of using remote resources with SmartIO is comparable to that of local access (in terms of latency and throughput).
    
    \item Demonstration of the real-world capabilities of SmartIO, by testing it on realistic large-scale workloads (see \cref{paper:mmsys}, \cref{paper:cc}, \cref{paper:tocs}). Two different image classification workloads relying on multiple \glspl{gpu} and \gls{nvme} storage have been tested with SmartIO, in order to prove that it is a viable solution for cost-effective scaling of \gls{io} resources.
