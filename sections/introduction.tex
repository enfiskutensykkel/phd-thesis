...
\begin{itemize}
	\item Improvements to the Device~Lending mechanism~\cite{Kristiansen2015}, and the integration of this mechanism into the SmartIO solution.
		Device~Lending makes it possible to distribute \glspl{nvme}, \glspl{gpu}, network adapters, and any standard \gls{pcie} device to remote systems and used by existing application software and device drivers without requiring any modifications.
		The improvements to this mechanism include:
		\begin{itemize}
			\item Developing support for multiple devices and enabling \gls{p2p} between them.
			\item Identifying performance bottlenecks and improving the performance to the point where using remote resources becomes comparable to local access
			\item Development of an interrupt forwarding mechanism that makes it possible to support share \gls{sriov} \glspl{vf} to different machines on newer Linux kernels.
		\end{itemize}

	\item Extending Device~Lending to \glspl{vm} by developing an interface extension to \gls{kvm} based on \gls{mdev}.


\end{itemize}

%In short, SmartIO is a flexible framework for device distribution and resource sharing that enables cost-effective and efficient scaling of resources between PCIe-networked machines.

The work of this thesis has been presented in five peer-reviewed publications:
\begin{itemize}
	\item The Device~Lending mechanism has been presented in \cref{paper:nossdav}
\end{itemize}

The work of this thesis has been presented in five peer-reviewed publications: two conference workshop papers, one short-length paper (demo paper), and two journal articles.
%
These publications are included as \crefrange{paper:nossdav}{paper:tocs}.
%
Additionally, several components of SmartIO has already been incorporated into the product line of Dolphin Interconnect Solutions, and is seeing real-world deployments.
...
%Devices may be distributed to physical hosts and to \glspl{vm} alike, and SmartIO also provides facilities for \glslink{disaggregation}{disaggregating} devices and memory resources in software, similarly to \gls{rdma} solutions.
%%
%Unlike \gls{rdma} solutions, however, resources are accessed over native \gls{pcie}, allowing them to be shared without introducing a performance penalty.
%
%%
