...
To validate the solution, it must be thoroughly tested.
%
%
A wide range of functionality tests and performance experiments should be included in order to provide a complete evaluation:
%
\begin{itemize}
    \item In order to prove that any kind of \gls{pcie} device can be shared, different types of devices, like \glspl{nvme}, \glspl{nic}, and \glspl{gpu}, should be tested.
        %
        This also proves that the solution works for different device driver implementations.
        %
    ...
    \item The performance of the critical \gls{io} data path should be examined thoroughly through a wide range of latency and throughput benchmarks.
        %
        Since the solution allow machines to use the internal devices and memory in other (remote) machines as if these resources were locally installed, we can use comparison testing to establish a performance baseline for our solution.
        %
        A test can first be performed using local resources and then repeated for remote resources using our solution. 
        %
        If the results matches the expectations, then the test has passed.
        %
        By looking at individual \gls{io} operations, such as reading/writing to an \gls{nvme} or copying memory out of \gls{gpu} memory, any performance overheads in the different components of the implementation can be identified (and subsequently improved).
        %
    ...
    \item Realistic \gls{io}-intensive computing tasks, like a machine~learning job, should be used to stress test the solution.
        %
        By putting the solution under heavy load, we can observe the accumulated effects of any small performance overheads caused by the implementation that are not visible on their own.
        %
        It can also reveal any side-effects the solution has on the systems themselves.
        %
        Showing that the solution works for a realistic workload not only proves that it is stable and gives reliable performance, but it also demonstrates that scaling real-life applications is possible.

    \item The evaluation should also explore possible optimizations that are enabled by our solution, such as exploiting memory locality to reduce latency or using \gls{pcie} \gls{multicasting} to replicate data across several machines.
        %
        This has the added benefit of showcasing several sharing scenarios that are made possible by the solution.
        %
        Furthermore, we can also use this to highlight sharing capabilities that other solutions lack, for example showing that CUDA unified memory~\cite{url:unified-memory} can be supported, even when \glspl{gpu} reside in different machines.

\end{itemize}
%
To prove that the solution is general, the evaluation should rely on standard (and unmodified) hardware and software for tests.
%
We make a point of using widely available benchmarking tools and application software, as well as standard device drivers.
%
