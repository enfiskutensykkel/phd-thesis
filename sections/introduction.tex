...
In order to meet the challenging latency and throughput requirements of data-driven and compute-heavy workloads, there is a need for flexible, yet efficient, sharing of \gls{io} resources in a networked computing cluster.
%
...
%
By leveraging memory mapping functionality supported by the \gls{pcie} networking hardware, our solution makes it possible to use resources residing in remote machines in a manner that is indistinguishable from using resources attached to the local \gls{pcie} bus.
%
Whether resources are local or remote is made transparent to application software, \gls{os}, and even device drivers.
%
Not only does this make it possible to increase the overall resource utilization in the cluster, but it becomes easier to implement distributed applications.
%
Application software no longer needs to be written with accessing remote resources in mind, but can be implemented as if all resources are local.
%
Using our solution, \gls{io} resources are no longer locked to individual machines as memory and \gls{pcie} devices can be shared freely with other machines in the cluster.



\section{Background and motivation}\label{sec:motivation}
In cloud computing environments, dynamically scaling and partitioning \gls{io} resources is often handled through virtualization. 
%
\Gls{vm} \glspl{hypervisor} may dynamically add virtual \gls{io} devices to \gls{vm} instances on demand.
%
It is even possible to temporarily suspend computation to migrate \glspl{vm} to \gls{host} machines with more hardware resources available, should the requirements of a \gls{vm} \gls{guest} exceed the available local physical resources.
%
...
%
However, although \gls{passthrough} allows physical hardware to be used with minimal software overhead, this technique suffers from a lack of flexibility as the physical devices are tightly coupled with the hosts they are installed in.
%
...
%There is a need for a solution that both enables sharing on a more fundamental level than \gls{rdma} implementations, and provides greater flexibility than what is currently possible using virtualization.
%
%This dissertation contributes to this goal by presenting a system for sharing and distributing \gls{io} devices among cluster machines interconnected with \gls{pcie}.
%
%By leveraging the shared-memory functionality supported by the \gls{pcie} interconnection hardware, using memory and devices in remote machines becomes indistinguishable from using resources attached to the local \gls{pcie} bus.
%
%Application software no longer needs to be written with accessing remote resources in mind, but can be implemented as if all resources are local.
%
%Furthermore, by relying on native \gls{pcie}, our system not only supports sharing of all types of \gls{pcie} devices, but also allows remote resources to be accessed with performance comparable to local access.
%
%Finally, as our system enables resource sharing at multiple abstraction levels, devices may be distributed to remote host~machines and \glspl{vm} alike.
%
%Devices and memory may even be \lgls{disaggregation}{disaggregated} and shared with several machines simultaneously, without requiring virtualization support in device hardware or a complex software infrastructure.
%
%Using our system, memory and \gls{pcie} devices are no longer locked to individual machines and resources can be shared freely with other machines in the cluster without requiring virtualization or \gls{rdma}. 
%
%The overall resource utilization in the cluster is increased, making it easier to meet the latency and throughput requirements of data-driven and compute-heavy cluster applications.




\Gls{pcie} is the de~facto standard for connecting devices to a computer system~\cite{Fountain2005}.
%
...
\begin{figure}
	\centering
    \includegraphics[width=.97\linewidth]{cluster-example}
...
By far, the most common way of translating addresses between different \gls{pcie} address domains is by using a special type of device called \gls{ntb}~\cite{whitepaper:PLX,whitepaper:Regula2004,Hou2013,Tu2013,Tu2014,Tu2018}.
%
...
%
Using such \gls{ntb}-capable switch chips to implement peripheral devices, it becomes possible to connect independent computer systems with plug-in adapter cards and external cables~\cite{Ravindran2008,Shim2018,Lim2019,Tu2018}.
%
As depicted in \cref{fig:cluster-example}, in such \gls{pcie} networks, both \glspl{cpu} and internal \gls{pcie} devices are attached to the same, shared \gls{pcie} fabric.
...
%
Remote resources, such as memory and \gls{io} devices, can be mapped into a local system and accessed through the \gls{ntb} with very little performance overhead.
%
Similarly, a remote device capable of \gls{dma} may also use the \gls{ntb} to access local resources.
%
No conversion between the network and the \gls{io} bus is required, since all memory address translations can be done in \gls{ntb} hardware.
%
There is no need to use memory on the remote system as an intermediate step when transferring data, and software in the critical data path can be avoided entirely as illustrated in \cref{fig:direct-access}.
...
\begin{figure}
    ...
    \begin{subfigure}{\linewidth}
        ...
        \caption{Accessing remote resources using \glsxtrshort{rdma}.}
    \end{subfigure}
    \par\vspace{5mm}
    \begin{subfigure}{\linewidth}
        \centering
        \includegraphics{direct-access-smartio}
        \caption{Accessing remote resources over native \glsxtrshort{pcie}.}
    \end{subfigure}
    \caption
    [Remote hardware can be accessed directly without any software in the critical path by using \glsfmtshortpl{ntb}]
    {Many \gls{disaggregation} solutions have performance overheads because they rely on \gls{middleware} or other forms of software facilitation on the remote system. By setting up memory mappings through the \glsxtrshort{ntb}, remote hardware can be accessed directly without any software in the critical path.}
    \label{fig:direct-access}
\end{figure}



Setting up such \gls{ntb} mappings,~however, requires awareness of the address space on the remote system.
%
...
discovering device
%
This address space awareness greatly increases the programming complexity of device drivers. 
something something virtualization, but owned by a single host. cite ladon, huo


Implementing \gls{disaggregation} 



\gls{ntb}-based \gls{disaggregation} 



%olutions for \gls{pcie} not based on \glspl{ntb}

%By relying on virtualization functionality in \gls{pcie} switch chip hardware to dynamically partition the \gls{pcie} fabric, 
%
...
%\cite{url:liqid,url:rackscale,url:gigaio}
%PCIe-based solutions rely on additional virtualization functionality in the PCIe switch
%chip hardware to partition the PCIe fabric and create virtual device trees for each individual host~\cite{chung2018,microsemi2019}. 
%%
...
%Some \gls{pcie}-based \gls{disaggregation} approaches using NTBs have been proposed in the past~\cite{Hou2013,tu-2014}, 
...
%
%While some \gls{disaggregation} approaches using \glspl{ntb} have been proposed in the past~\cite{Hou2013,Tu2014}, 


%\cite{Chung2018,Microsemi2019}. 
%these implementations present solutions where devices are owned by a dedicated server. As distributing
%resources is generally only possible to hosts that are directly connected to the same switch as this
%server, these approaches forgo the flexibility of fully distributed cluster computing systems.
%Alternative PCIe-based solutions rely on additional virtualization functionality in the PCIe switch
%chip hardware to partition the PCIe fabric and create virtual device trees for each individual host~
%%
%These solutions allow devices to be directly attached a switch rather than a server. 
%%However, distributing devices is still only possible to hosts that
%%are directly connected to the same switch.
