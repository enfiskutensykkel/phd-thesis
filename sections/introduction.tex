...
%
The sharing capabilities of an \gls{rdma} solution is, therefore, often limited to a single (type of) device. 
Sharing different types of devices, for example both \glspl{gpu} and \glspl{nvme}, not only requires multiple, distinct \gls{disaggregation} implementations, but integrating these different implementations with each other may also be a challenge.
...
%
This dissertation contributes to this goal by presenting a system for sharing resources and distributing devices in a heterogeneous cluster interconnected with \gls{pcie}.
%
%We have
...
\begin{figure}
	...
	\caption{TODO}\label{fig:device-pool}
\end{figure}
...
\begin{figure}
    ...
    \end{subfigure}
    \caption[Remote hardware can be accessed directly without any software in the critical path by using \glsfmtshortpl{ntb}]{Many \gls{disaggregation} solutions have performance overheads because they rely on \gls{middleware} or other forms of software facilitation on the remote system. 
    By setting up memory mappings through the \glsxtrshort{ntb}, remote hardware can be accessed directly without any software in the critical path.}\label{fig:direct-access}
\end{figure}
