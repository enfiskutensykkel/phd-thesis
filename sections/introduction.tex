...
%
Even though \gls{passthrough} allows the device to be used directly by a device driver running in a \gls{guest}, requiring that compute tasks run in \glspl{vm} inevitably adds additional system load, as \gls{cpu} cycles are spent on hosting the virtualized environment.
%
...
Nevertheless, this kind of abstraction gives rise to another challenge; a device driver that is unaware that a device is remote may assume that the entire local address space can be reached by the device.
%
...
%
Deferring mappings until the device driver initiates \gls{dma} would require synchronizing with the remote system, thus increasing the overall latency.
%
A mechanism for preparing the necessary memory-mappings in advance, without introducing any communication overhead in the critical path, is needed.
