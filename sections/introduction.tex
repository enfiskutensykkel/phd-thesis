...
\begin{figure}
    ...
    \begin{subfigure}{\linewidth}
        ...
        \caption{Accessing remote resources using \glsxtrshort{rdma}.}\label{fig:direct-access-rdma}
    \end{subfigure}
    \par\vspace{5mm}
    \begin{subfigure}{\linewidth}
        \centering
        \includegraphics{direct-access-smartio}
        \caption{Accessing remote resources over native \glsxtrshort{pcie}.}\label{fig:direct-access-smartio}
    \end{subfigure}
    \caption[Remote hardware can be accessed directly by using \glsfmtshortpl{ntb}]{Many \gls{disaggregation} solutions have performance overheads because they rely on \gls{middleware} or other forms of software facilitation on the remote system. In \glsxtrshort{pcie} networks, remote hardware can be accessed directly without any software in the critical path by setting up memory mappings over the \glsxtrshort{ntb}.}\label{fig:direct-access}
\end{figure}
