...
%
Not only does this make it possible to increase the overall resource utilization in the cluster, but it also becomes easier to implement distributed applications.
%
...
%
This makes it possible to scale out and use more hardware resources than there are available in a single machine, increasing the performance of the entire cluster.



\section{Background and motivation}\label{sec:motivation}
In cloud computing environments, dynamically scaling and partitioning of \gls{io} resources is often handled through virtualization. 
%
...
%
As such, \gls{passthrough} allows physical hardware to be used by a \gls{vm} \gls{guest} with minimal to no software overhead.
%
...
Due to its very low latency overhead, using \gls{pcie} as a high-speed interconnection technology is a compelling alternative to traditional networking technologies.
%
\Gls{pcie} is the standard for connecting \gls{io} devices to a computer system~\cite{Fountain2005}.
%
Extending the \gls{pcie} bus out of a single computer in order to interconnect several computer systems is particularly interesting, as it would create a network where all \glspl{cpu} and internal \gls{pcie} devices would be attached to the same, shared \gls{pcie} fabric~\cite{url:Meduri2011,whitepaper:Regula2004,Ravindran2008}. 
%
Unlike distributed \gls{io} in a traditional network, remote resource access in a \gls{pcie} network would have a clear performance advantage, since no conversion between the network protocol and the \gls{io} bus is necessary.



Since \gls{pcie} was designed as a local \gls{io} bus standard, connecting devices to the \gls{cpu} on a motherboard, individual computer systems operate with independent \gls{pcie} address domains.
%
State of the art \gls{pcie}-based disaggregation solutions rely on virtualization support in \gls{pcie} switch chip hardware to dynamically partition a shared \gls{pcie} fabric, creating a logical device tree for each individual machine that maps to its address domain~\cite{Chung2018,whitepaper:Microsemi,url:rackscale,url:liqid,url:gigaio}. 
%
Rather than being installed into a machine, devices are instead attached directly to a so-called ``top of rack'' switch (or a series of such switches).
%
Devices can then be logically assigned and reassigned to different machines on the fly.
%
It is even possible to distribute individual \glspl{vf} of devices that support \gls{sriov} to different machines.
%
However, by partitioning the fabric into separate, logical address domains, these \gls{disaggregation} solutions stops short of supporting communication between machines. As such, they forgo the possibility of supporting true \gls{pcie} networking.
...
Building a cluster of \gls{pcie}-interconnected machines that supports memory-to-memory communication is desirable because of the intrinsic memory addressing capabilities of \gls{pcie}~\cite{Shim2018,Lim2019}.
%
This requires translating memory transactions between the address domain of one machine to another.
%
...
%
Using such \gls{ntb}-capable switch chips to implement peripheral devices, it becomes possible to connect independent computer systems with plug-in host adapter cards and external cables, as illustrated in \cref{fig:cluster-example}.
%
...
%
Since all memory address translations would be done in \gls{ntb} hardware, this approach eliminates the need to use memory on the remote system as an intermediate step when transferring data. 
%
Software in the data transfer path can also be avoided entirely, as depicted in \cref{fig:direct-access}.
%
...
%
Hence, relying on \glspl{ntb} alone for distributed \gls{io} is not a viable solution.
%
Not only does it require extensive modifications to existing device driver software in order to manage multiple address space layouts, but the programming complexity greatly increases when the number of machines in the cluster increase beyond a few nodes.



%Existing \gls{pcie}-based \gls{disaggregation} solutions rely on virtualization to mitigate the complexity of managing different \gls{pcie} address spaces.
To mitigate the complexity of managing different \gls{pcie} address spaces, the Ladon system~\cite{Tu2013} relies on virtualization.

%
In the Ladon system~\cite{Tu2013}, devices can be distributed to \gls{vm} \glspl{guest} running on host machines that are directly connected to the same \gls{ntb}-capable switch.
%
Devices are owned by a dedicated ``management host'' that maps the memory of each 
%
By mapping the memory of each host machine, 
the server assists the \gls{hypervisor} on each host machine in setting up device mappings.

dedicated server, forgo 

the Ladon system~\cite{Tu2013} relies on facilitation from \gls{vm} \gls{hypervisor} to assist \gls{vm} \glspl{guest}

%Existing \gls{pcie}-based solutions solve this through virtualization, either by relying on facilitation from hypervisors~\cite{Tu2014} or 


Implementing \gls{ntb}-based \gls{disaggregation} 
