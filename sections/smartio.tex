...
%
A more technical comparison between specific solutions and our SmartIO solution and is provided in \paperref{tocs:rw}.
...
\begin{itemize}
    \item So-called ``rack-scale \gls{disaggregation}'' and device sharing by \textbf{logical partitioning}, where modular blade servers are connected to a backplane or a shared \gls{io} bus fabric within a data center rack.
        ...
        %
        Instead, additional \gls{disaggregation} solutions are needed in order to support this, for instance \gls{rdma}-based solutions.
    ...
    \item Device \gls{disaggregation} by \textbf{distributed \gls{io} over a network using \gls{rdma}}.
        ...
        %
        Solutions for sharing \glspl{gpu}~\cite{Hou2013} and Ethernet \glspl{nic}~\cite{Tu2014} over a \gls{pcie} fabric using \gls{rdma} have also been proposed, by using \gls{pcie} switch chips capable of \gls{dma} to transfer data from memory to memory in the different machines.
    ...
    \item Memory \gls{disaggregation} through \textbf{remote memory access}.
        %
        This category includes solutions that require the use of parallel programming models on top of \gls{rdma}, such as \gls{mpi}~\cite{Jiang2004}, \gls{rpc}~\cite{Lu2013}, or through other abstractions~\cite{Aguilera2018}.
        %
        Approaches for accessing remote memory in a \emph{transparent} fashion also exist, most commonly realized by ``\lgls{trap}{faulting in}'' remote memory pages by modifying the system page fault handler to initiate \gls{rdma} transfers~\cite{Liang2005,Gu2017,Lim2009}.



        Memory \gls{disaggregation} solutions, in particular those that use explicit programming models, are not so much aimed at sharing \gls{io} resources, but rather enabling shared-memory communication for distributed cluster applications.
        %
        Especially \gls{mpi} protocols implemented using \gls{rdma} appears to be widely used for this purpose.
        %
        Many of these solutions also support \gls{gpudirect}, effectively making it possible to \gls{disaggregate} \gls{gpu} memory~\cite{Venkatesh2014,Venkatesh2017,url:Rosetti2014}.
        %
        ...
        %
        compared to a \gls{cpu} (or device) reading and writing to a memory location directly, latency is significantly increased by initiating \gls{rdma} transfers.
\end{itemize}
