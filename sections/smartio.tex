...
\begin{figure}
    ...
    \caption[SmartIO supports three different sharing methods that rely on the same the address resolving mechanism]
    {SmartIO supports three different sharing methods that rely on the same address resolving mechanism for translating \glsxtrshort{io} addresses between different address spaces.}
...
\begin{description}
    \item[Low-level \gls{ntb} driver]
        %
        SmartIO is built on top of the \gls{ntb} interconnection solution from Dolphin.
        %
        The low-level \gls{ntb} driver provides a \gls{pcie} cluster networking infrastructure as well as basic shared-memory functionality which SmartIO builds on.
        %
        This functionality includes mapping remote memory segments into local address space, and allocating such segments in local memory and making them available for other, remote machines.
        %
    ...
    \item[Address Space Abstraction Layer]
        %As mentioned in \cref{sec:idea}, the basic memory mapping capabilities of \glspl{ntb} are by themselves insufficient for enabling distributed \gls{io} and resource sharing.
        %
        SmartIO implements an abstraction layer that manages the complexity of dealing with multiple \gls{io} address spaces.
        %
        This layer provides a logical decoupling of resources,~i.e., devices and memory segments, and which physical machines they are installed in. 

        making it possible to abstract away 

        ssists in setting up necessary \gls{ntb} mappings for the borrowing system and resource
