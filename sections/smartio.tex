...
\begin{itemize}
    ...
    \item We perform \gls{p2p} experiments with \glspl{gpu} in several network topologies in \paperref{srmpds:eval}, evaluating the effect longer \gls{pcie} paths have on \gls{dma} performance. These tests are extended in \paperref{cc:eval} with additional network topologies and test scenarios.
        ...
        %
        In order to demonstrate \gls{dl} and \gls{mdev} for real-world applications, we also evaluate the run-time performance of a machine~learning workload using borrowed \glspl{gpu} and an borrowed \gls{nvme} in \paperref{cc:eval-ml}.

    \item An exhaustive performance evaluation covering \emph{all} parts of the final implementation of SmartIO is presented in \paperref{tocs:eval}.
        %
        Here, we evaluate SmartIO from all angles using multiple hardware configurations and devices, in order to prove that when conditions are similar, our SmartIO sharing methods \textbf{do not add any performance overhead} compared to using local resources.
        %
        Additionally, \paperref{tocs:eval-nvme} also present several tests of our proof-of-concept \gls{nvme} driver, including an experiment with 30 cluster nodes sharing a single-function \gls{nvme} simultaneously.
\end{itemize}
...
%
The third test is intended to demonstrate memory optimization functionality in our proof-of-concept \gls{nvme} driver enabled by the \gls{apiext}, by making it possible to allocate \glspl{sq} in different kinds of memory (both locally and remote).



All three tests involve two Intel~Xeon machines, and PXH830 \gls{ntb} adapter cards from Dolphin~\cite{pxh830}.\footnote{Additional \gls{pcie} network topologies are evaluated in \crefrange{nossdav}{tocs}, including larger clusters.}
%
Additionally, all three tests use a BP-457-ATX \gls{pcie} expansion chassis from One~Stop~Systems in order to create a \gls{pcie} topology that is similar for local and remote test runs.
%
The expansion chassis adds the same number of \gls{pcie} switch chips (or ``hops'') in the path between the \gls{cpu} and the device in both the local and remote scenarios.\footnote{We show how longer \gls{pcie} paths affects \gls{dma} performance in \paperref{cc:eval-native-p2p}.}
%
Moreover, these switch chips are the same Broadcom PEX8733~\cite{pex8733} switch chips used to implement the \gls{ntb} adapter cards.
%
Note that while the hardware configuration is similar for all three tests, we have used different Linux distributions.
%
This is to demonstrate that our SmartIO framework is not limited to a specific Linux version, but works for a wide variety of distributions and software versions, including older kernels.
%
We make a point of using standard and unmodified benchmarking software for our tests.



\subsection{NVMe latency comparison}
...
%
%consisting of both synthetic microbenchmarks that and realistic, large-scale workloads:
%%
%
%First, we 
establishing a local ``baseline'' for expected performance.
%%
...
\begin{figure}
    ...
    \begin{subfigure}{\linewidth}
        ...
        \caption{\textbf{Local Baseline}: aa}
    \end{subfigure}
    \par\vspace{5mm}
    \begin{subfigure}{\linewidth}
        \centering
...
\subsection{GPU throughput comparison}
\begin{figure}
    \centering
    \begin{subfigure}{\linewidth}
        \centering
