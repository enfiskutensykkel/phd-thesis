...
\begin{figure}
    ...
    the \gls{cpu} to read from and write to device memory the same way it would access \acrshort{ram}.}
...
%
As such, the term ``\gls{bar}'' is used synonymously for a device memory region.
%
Moreover, devices are also made aware of addresses reserved by the system for interrupts through their configuration space registers.


\begin{figure}
    \centering
    \includegraphics{ntb-example}
    \caption[Two computer systems connected using \glsfmtshortpl{ntb}, and the \glsfmtshortpl{ntb} translates between the two different address domains]
    {Two computer systems connected together using \glspl{ntb} and external cables. Host~1 has mapped segments of Host~2's memory through its local \gls{ntb}, providing Host~1 with ``windows'' into the remote system's address space. The \glspl{ntb} translates addresses between the two domains.}
    \label{fig:ntb-example}
\end{figure}

Since this enumeration and address reservation process is typically done when a system starts up, different systems the address space layouts will vary from system to system.
%
However, it is possible to connect computer systems with independent address domains together over \gls{pcie} by using \glspl{ntb}.
%
...
%
Using such \gls{ntb}-capable switch chips to implement peripheral devices and cluster switches, it becomes possible to connect independent computer systems with plug-in host adapter cards and external cables as depicted in \cref{fig:ntb-example}.
