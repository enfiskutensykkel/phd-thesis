...
\section{Principal idea}\label{sec:idea}
\begin{figure}
    \centering
...
\begin{figure}
    ...
    \caption[Three different sharing methods are made possible by our framework]
    {The software architecture of SmartIO. Three different sharing methods are made possible by our framework: \textbf{(1)}~Device~Lending,\textbf{(2)}~\glsxtrshort{mdev}, and \textbf{(3)}~using the \gls{sisciapi}~extension.}
...
%
\Cref{fig:architecture} illustrates the different software components of the implementation and how they fit together.
...
%
It provides a logical decoupling of devices and which physical hosts they are installed in.
%
...
Most importantly is the SmartIO driver's capability of mapping memory segments on behalf of a device.
%
SmartIO resolves paths through the cluster network between a device and a memory segment in any machine, and dynamically sets up the necessary \gls{ntb} maps for the device through the appropriate \glspl{ntb}.
%
A borrower can retrieve a device-side \gls{io} addresses that maps through the device-side (or lender) \gls{ntb} to mapped memory segments.
%
This makes it possible for a borrower to use remote \gls{io} addresses without requiring that it is aware of the underlying, physical \gls{pcie} topology or the internal \gls{io} address space layout of remote machines.
%
As such, the SmartIO driver acts as an abstraction layer, allowing software on borrowers to remain agnostic about remote address spaces.
%
With this abstraction, SmartIO is able to facilitate sharing and use of remote resources (both memory and devices).
