...
Related \gls{disaggregation} solutions can be broadly divided into the following four groups:
%
\begin{itemize}
    ...
    \item \textbf{\gls{rdma}-based resource sharing}.
        ...
        As we also addressed in \cref{sec:motivation}, \gls{rdma} is not as closely integrated to the \gls{pcie} fabric as our own \gls{ntb}-based solution is.
        %
        Even though \gls{rdma} enables efficient transfer over a network, device \gls{disaggregation} solutions based on \gls{rdma} typically require application- or device-specific \gls{middleware}, or another method of communicating with a driver running on the \emph{device-side} system, in order to interact with the remote device.
        %
        This additional software complexity inevitably leads to a performance overhead, compared to accessing a device on the local \gls{pcie} bus where only a local device driver is involved.
        %
        In comparison, our SmartIO framework enables access to remote resources over native \gls{pcie}, allowing remote devices to be used with near-local performance (\cref{sec:eval}).
        %
        Furthermore, SmartIO is general, in the sense that it is not only possible to share \emph{any} \gls{pcie} device, but we are able to enable direct access to memory resources (both system memory and device memory).
        %
        This makes it possible to support features such as \gls{cuda} unified memory~\cite{url:unified-memory}.
        %
        We are not aware of any \gls{rdma}-based \gls{gpu} \gls{disaggregation} solutions that are able to support this.

    \item Shared-memory \Gls{pcie}-based shared memory approaches
        also ntb shared memory thing, marlin
        Lim2009
...
ladon as a separate point here
