...
\subsection{Device Lending: Latency experiment}
\begin{figure}
    \centering
    \begin{subfigure}{\linewidth}
        \centering
...
\subsection{Device Lending: Throughput experiment}
\begin{figure}
    \centering
    \begin{subfigure}{\linewidth}
        \centering
...
Compared to other solutions implemented using \glspl{ntb}, SmartIO is a more comprehensive framework for sharing devices and memory resources in a \gls{pcie} network.
%
Our solution make it possible to share devices to remote machines, both bare-metal \glspl{hostmachine} and \glspl{vm}.
%
Through the SmartIO abstraction layer, the hard distinction between local and remote is blurred out not only for native device drivers, but we are also able to combine our device sharing capabilities with shared-memory functionality through the \gls{sisciapiext}.
%
This unlocks a new potential for machines connected over \gls{pcie}, as it not only becomes easier to scale out and use remote \gls{io} resources, but traditional \gls{io} may even be combined with distributed, shared-memory functionality.
