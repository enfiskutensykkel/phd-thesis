...
The published papers provide further details on the challenges facing our framework.
%
As part of the description of the implementation of the different components of SmartIO, described in \paperref{tocs:lending,tocs:mdev,tocs:nvme}, we provide a more in-depth explanation of the main challenges and how we have tackled them.
%
We also provide a discussion of additional considerations that are not mentioned here in \paperref{tocs:disc}.
...
\begin{figure}
    ...
    {The software architecture of SmartIO. Three different sharing methods are made possible by our framework: \textbf{(1)}~Device~Lending,\textbf{(2)}~\glsxtrshort{mdev}, and \textbf{(3)}~using the \gls{sisciapi}~extension. The SmartIO driver, shown as \textbf{(B)}, abstracts away the physical location of remote resources, for both the physical device and software using the device.}
...
%
This also explains how memory regions of \emph{other} devices can be mapped for a device, even when they reside in a different lender machine.
...
\begin{figure}
    ...
    {Device~Lending injects a hot-added ``shadow device'' into the Linux kernel device tree using mappings to \glspl{devicebar} and }
    \label{fig:device-lending}
\end{figure}

Illustrated as arrow \textbf{(1)} in \cref{fig:architecture}.
