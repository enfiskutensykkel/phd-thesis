...
\subsection{Low-level \glsfmtshort{nolist-ntb} driver}\label{sec:ntb-driver}
...
\subsection{\Glsfmttext{dl}}\label{sec:lending}
...
\subsection{\Glsfmtshort{nolist-mdev}}\label{sec:mdev}
...
\subsection{\Glsfmtshort{api} extension}\label{sec:api}
...
\subsection{Proof-of-concept \glsfmtshort{nvme} driver}\label{sec:nvme-driver}
...
\subsection{\Glsfmttext{dl}: latency comparison}\label{sec:eval-lat}
\begin{figure}
    \centering
    \begin{subfigure}{\linewidth}
        \centering
...
\subsection{\Glsfmttext{dl}: throughput comparison}\label{sec:eval-bw}
\begin{figure}
    \centering
    \begin{subfigure}{\linewidth}
        \centering
...
\subsection{Proof-of-concept \glsfmtshort{nvme} driver experiment}\label{sec:eval-nvme}
...
\begin{figure}
    ...
    \begin{subfigure}{\linewidth}
        ...
        \caption%
        {Our proof-of-concept \glsfmtshort{nvme} driver is implemented as part of the application process, running on the borrower's \glsxtrshort{cpu} and operating the remote \glsxtrshort{nvme}. The remote \glsxtrshort{gpu} is borrowed using \gls{dl}, and both \glsxtrshortpl{gpu} are operated using the native \gls{cuda} driver. Data is loaded from storage directly into \glsxtrshort{gpu} memory using a random read access pattern.}
...
%
To demonstrate this, we have designed the following experiment (as illustrated in \cref{fig:eval-nvme-design}):
%
\begin{itemize}
    \item 
        %
        We developed a \gls{userspace} \gls{cuda} application and included our proof-of-concept \gls{nvme} driver as part of this application.
        %
    ...
    \item 
        ...
        \end{description}
        %
        For more context on why \gls{dma} reads are affected by longer distances, please refer to \paperref{tocs:eval-lending-path}.
    ...
    \item 
        ...
        %
        However, both \glspl{gpu} are included as destinations in our experiment, for the purpose of showcasing the capabilities of SmartIO.\footnote{A similar experiment is also presented in \paperref{tocs:eval-nvme-sq}, but here only the local \gls{gpu} is used as a data destination.}
...
Our performance results show that while there is an associated development cost of implementing device drivers using the SmartIO \gls{apiext} (and building them into the application), there is a performance advantage compared to using \gls{dl} and native device drivers.
%
Our SmartIO \gls{apiext} makes it possible to optimize data flow through the cluster network, for example by streaming data directly into \gls{gpu} memory as shown in this experiment.
%
By integrating driver operation with the \gls{sisciapi}, additional functionality also become possible, like using \gls{pcie} \gls{multicasting} to replicate data across several cluster machines in a single operation.
%
Replicating \gls{nvme} data across 60~machines using \gls{multicasting} is demonstrated in \paperref{tocs:eval-nvme-sharing}.


% TODO: Conclude experiments section with a paragraph that summarizes both zero overhead ("remote and local performance are equal")  and API extension/NVMe driver stuff
...
\subsection{Solutions not using \glsfmtshortpl{nolist-ntb}}
...
\subsection{Solutions using \glsfmtshortpl{nolist-ntb}}
