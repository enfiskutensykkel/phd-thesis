...
need an idea for an intro here

\section{General idea}\label{sec:idea}
\begin{itemize}
    \item a combined (and shortened) version of PCIe overview + NTB + mapping bars and DMA windows from previous papers
    \item simple ntb figure for mapping memory on remote node
    \item the famous device lending figure
    \item the general idea: can stuff (memory + bars) be mapped through the NTB in a way that is transparent for OS?
\end{itemize}
