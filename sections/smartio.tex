...
Resource \gls{disaggregation} and sharing is a wide-ranging topic that covers several research areas.
%
In fact, each individual component of SmartIO could potentially be discussed at great length on their own, in order to place them in proper context.
%
...
At its core, SmartIO is a framework for sharing \gls{io} resources among machines in a network.
%
Related resource \gls{disaggregation} and sharing solutions can broadly be divided into the following two categories:
%
\begin{itemize}
    \item Solutions based on \textbf{\Gls{pcie} switch partitioning}, including so-called ``rack-scale computing'' solutions~\cite{url:rackscale,url:liqid,url:gigaio}.
        %
        These solutions rely on \gls{pcie} switch chips with virtualization support to create logical device trees (as illustrated in \cref{fig:partitioning})~\cite{whitepaper:Microsemi,Wong2011,whitepaper:IDT,Chung2018}.
        %
        We also include \gls{mriov}~\cite{spec:MRIOV} in this category, although we are not aware of any commercially available implementations of it.
        
        As already addressed in \cref{sec:motivation}, by relying on partitionable switches as the enabling component, these solutions are only share devices that are directly attached to such switches.
        %
        It is not possible to share the \emph{internal} devices of a machine.
        %
        Moreover, solutions based on switch partitioning do not specify any memory-to-memory communication between hosts, requiring alternative networking technologies instead.
        %
        Consequently, partitioning does not provide the level of sharing compared to our \gls{ntb}-based system.
        %
        Simultaneously sharing a (non-\gls{sriov}) device between multiple machines requires additional distribution methods, such as \gls{rdma}.
        %
        In contrast, our SmartIO implementation is fully distributed, allowing all machines to contribute their own internal devices, and use the internal devices of other machines.
        %
        Shared-memory computing is made possible by \glspl{ntb}.
        %
        Using the \gls{sisciapiext}, it is even possible to implement ``\gls{mriov} in software'', as demonstrated by our proof-of-concept \gls{nvme} driver (\cref{sec:nvme-driver}).
        

    \item Solutions based on \textbf{using \gls{rdma} for \gls{disaggregation}}.
        %
        \Gls{rdma} is by far the most widely used solution for \gls{disaggregating} resources and sharing them over a network.
        %
        Examples include rCUDA~\cite{Duato2010} for sharing \glspl{gpu}, and \gls{nvmeof}~\cite{Guz2018} for \glspl{nvme}.
        %
        \Gls{rdma} has also been used for \gls{disaggregating} memory~\cite{Liang2005,Gu2017,Aguilera2018}.

\end{itemize}


also ladon and pcie shared memory


Lim2009


IO device sharing, rcuda nvmeof
memory disaggregation
\Gls{rdma}-based solutions include rCUDA~ and 
