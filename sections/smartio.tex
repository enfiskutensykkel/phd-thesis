...
\begin{figure}
    ...
    \begin{subfigure}{\linewidth}
        ...
        \caption{Loading data directly into the memory of a \gls{gpu} residing in the \gls{borrower}.}
    ...
    \begin{subfigure}{\linewidth}
        ...
        \caption{Loading data into, and storing data from, memory of a borrowed \gls{gpu}.}
...
\begin{figure}
    ...
    \caption
    [By using access pattern hinting, it is possible to consider memory locality without being aware of the underlying \glsfmtshort{pcie} topology]
    {Our \gls{nvme} driver implementation relies on SmartIO to decide where \glspl{segment} containing queues should be allocated. By using access pattern hinting, it is possible to consider memory locality without being aware of the underlying \gls{pcie} topology.}
...
\paperref{tocs:nvme} explains the implementation of proof-of-concept \gls{nvme} driver in more details.
%
Here, we also explain how our implementation can support multi-path fail-over, and how \gls{pciemulticasting} can be used to replicate data loaded from storage across machines in a single operation.
%
Finally, we also explain how our \gls{nvme} driver implementation can run as a \gls{gpu} application, offloading the task of operating the \gls{nvme} to the \gls{gpu} itself.
