...
%
Therefore, our extension to the \gls{sisciapi}, depicted as arrow \textbf{(3)} in \cref{fig:architecture}, makes it possible for several machines to share the same device (\gls{function}), by facilitating \gls{disaggregation} in software instead.


As mentioned in \cref{sec:idea,sec:ntb-driver}, a \gls{userspace} application may memory-map \glspl{sharedsegment} into its own local address space using \gls{sisci}.
%
This makes it possible for application processes to read and write to remote memory, as if it was reading or writing to local \gls{ram}.
%
Our \gls{apiext} extends this functionality with device-oriented programming semantics and support functions, making it possible to rely on the core SmartIO capabilities, as described in \cref{sec:smartio-driver}, through the same shared-memory \gls{api} used to write cluster applications.
%
The \glspl{bar} of a device is exported as \glspl{sharedsegment} that may be mapped by the application, providing access to device registers and other device memory.
%
...
%
Similarly, the we also allow \glspl{memorysegment} to be mapped for a device.
%
As such, devices may use native \gls{dma} to access \gls{sharedsegment} directly.
%
...
%
We provide a more detailed description of the added functionality in \paperref{tocs:api}.
...
%
Since the \gls{apiext} is built on the underlying SmartIO driver which resolves paths in the cluster, software can be written in a way that does not need to consider whether resources are local or remote.
%
However, the trade-off from using our \gls{apiext} is that it requires implementing a new device driver. 
%
Implementing a driver from scratch typically entails a considerable engineering effort, and may not even be a viable option in many cases.
%
After all, the main strength of \gls{dl} and the \gls{mdev} implementation is precisely that they do not require any modifications to existing device drivers.



Even so, implementing device drivers as part of cluster applications may potentially be extremely useful for some application domains.
%
First, as previously pointed out, \lgls{function}{single-function} devices may be \gls{disaggregated} at the software level. 
%
This software-enabled \gls{disaggregation} is demonstrated by our proof-of-concept \gls{nvme} driver, described in \cref{sec:nvme-driver}.
%
Second, it becomes possible to write device drivers that fully utilize the shared-memory capabilities of \gls{pcie} networks.
%
For instance, applications may use \gls{pcie} multicasting to stream data to several destinations in a single operation, or exploit memory locality in order to optimize data flow through the network.
%
It is even possible to combine the \gls{apiext} with the other sharing methods of SmartIO, \gls{disaggregating} a \emph{borrowed} resource.
%
An example of this is would be a machine using \gls{dl} to borrow a remote \gls{gpu} capable of GPUDirect~\cite{url:GPUDirect,url:Rosetti2014}, and then using the \gls{apiext} to export \gls{gpu} memory as a \gls{sharedsegment}.
%
This would allow other machines and devices to read and write directly to the \lgls{borrower}{borrowed} \gls{gpu}'s memory.
%
Our proof-of-concept \gls{nvme} driver also demonstrates this.
