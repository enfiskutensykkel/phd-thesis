...
%
The SmartIO driver is also responsible for making \glspl{devicebar} available as shared memory segments, which allows machines to memory map device memory such as device registers into their local address space (\gls{mmio}).



Most important is the SmartIO driver's responsibility of mapping memory segments \textbf{on behalf of a device}.
%
SmartIO resolves the physical locations of devices and memory segments,~i.e., which machines they reside in and which \glspl{ntb} the device must use in order to reach the segment, and is able to map resources for a device through appropriate \glspl{ntb}.
%
Note that the memory segment can reside in the \gls{borrowermachine}, in the \gls{lendermachine}, or a third and different machine altogether.
%
...
%
A \gls{borrower} may use \gls{io} addresses seen by the device, without requiring that it is aware of the internal \gls{io} address space layout of \glspl{lender} or even be aware of which physical machine the \gls{lender} even is.
%
For example, a \gls{borrower} can request that SmartIO maps a memory segment in a different machine so that the device may use \gls{dma} to it.
%
The SmartIO driver will set up the necessary maps on the \gls{lender} and return the (device-side) \gls{io} address that the \gls{borrower} can then use when interacting with the device in order to initiate the \gls{dma} transfer.
%
The \gls{borrower} does not need to know anything about this resolved address, other than the fact that it is mapped through the \gls{lender}'s, or device-side, \gls{ntb} so that the device is able to reach the memory segment.
...
%
As such, the SmartIO driver is the first step towards solving the challenges described in \cref{sec:challenges}.



More details on how SmartIO resolves the paths between devices and other memory resources that must be mapped for the device are given in the papers, particularly in \paperref{tocs}.
%
\paperref{tocs:lending-p2p,cc:p2p} explain how memory regions of \emph{other} devices can be mapped for a device, even when the devices reside in different \glspl{lendermachine}.
%
...
\Gls{dl}, illustrated as arrow \textbf{(1)} in \cref{fig:architecture}, makes it possible to distribute devices to remote \glspl{hostmachine}.
%
The devices become part of the system they are distributed to, allowing application software, device drivers, and even the \gls{os} to use them as if they were locally installed.
%
No modifications to software is required.


\begin{figure}
    \centering
    \includegraphics[width=.99\textwidth]{device-lending}
    %\caption[Device~Lending inserts a shadow device into the local device tree, making it possible to intercept device interactions by a device driver and transparently inject addresses]
    %{\gls{dl} inserts a \gls{shadowdev} into the local device tree, making it appear as if the remote device has been hot-added to the system. This \gls{shadowdev} makes it possible to intercept device interactions by a device driver and inject addresses that are mapped through the \gls{ntb} in a transparent manner.}
    \caption{TODO needs caption}
...
\Gls{dl} solves this by inserting a ``\gls{shadowdev}'' into the local \gls{pcie} device tree.
%
This \gls{shadowdev} makes it appear as if the remote device has been hot-added to the local system, and makes it possible to intercept certain operations a device driver performs on a device such as setting up \gls{dma} buffers or configuring interrupt addresses.
%
