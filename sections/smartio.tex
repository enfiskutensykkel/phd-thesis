...
%
An evaluation of SmartIO in the form of a comprehensive collection of performance tests can be found in the published papers, ranging from microbenchmarks aimed at evaluating a specific component to realistic, large-scale workloads:
%
\begin{itemize}
    ...
    \item The improved \gls{dl} implementation with support for \gls{p2p} between devices is evaluated in \paperref{srmpds:eval-p2p}, where we perform \gls{p2p} experiments with \glspl{gpu} in several network topologies and test scenarios, demonstrating the performance benefit of \gls{p2p} \gls{dma} compared to bouncing data via \gls{ram}.
        ...
        %
        We also show a \gls{sriov}-capable \gls{nic} being shared with multiple machines at the same time in \paperref{tocs:eval-sriov}.


    \item Our \gls{mdev} implementation is evaluated in \paperref{srmpds:eval-mdev}, measuring the performance of a \gls{vm} using a \lgls{passthrough}{passed-through}.
        %
        The impact of \glspl{iommu} on \gls{dma} transfers is demonstrated in \paperref{tocs:eval-iommu}.
        %
        In \paperref{tocs:eval-mdev-vfio}, we compare the performance of our \gls{mdev} implementation to ``regular'' \gls{passthrough} (of a local device) as well as bare-metal \gls{dl} (with the \gls{iommu} enabled).
        %
        Additional \gls{p2p} experiments using \gls{mdev} are also presented in \paperref{cc:eval-mdev-p2p}.
        


    \item To prove that \gls{dl} and \gls{mdev} can be used for real-world applications, we evaluate the run-time performance of a machine~learning workload using borrowed \glspl{gpu} and a borrowed \gls{nvme} in \paperref{cc:eval-ml,tocs:eval-ml}.
        %
        To further demonstrate \gls{dl} and \gls{mdev} being used for realistic applications, we present a \gls{gpu}-based video processing workload in \paperref{mmsys}, and use \gls{dl} to scale up the number of available \glspl{gpu} for the workload, in order to meet a real-time deadline.
    ...
    \item Several experiments using our our proof-of-concept \gls{nvme} driver are presented in \paperref{tocs:eval-nvme}, demonstrating capabilities made possible through the \gls{sisciapiext}.
        ...
        %
        A comparison of our proof-of-concept \gls{nvme} driver and a state of the art \gls{nvmeof} solution is presented in \paperref{tocs:eval-nvmeof}.
\end{itemize}
%
A complete performance evaluation of SmartIO in its entirety is presented in \paperref{tocs:eval}. 
%
Here, all parts of the implementation is evaluated from multiple angles, as well as the three sharing methods.
%
This evaluation also uses a wide variety of different Linux distributions, benchmarking software, and different \gls{pcie} devices, in order to demonstrate that our SmartIO framework is not limited to a specific Linux version or any specific device, but works for a wide variety of different software versions and devices.
%
...
In this section, we present shortened versions of three selected performance tests from this evaluation.
%
The first two tests, presented in \cref{sec:eval-lat,sec:eval-bw} respectively, are comparison tests.
%
To prove that our SmartIO framework is able to facilitate the use of remote \gls{pcie} devices with the same performance as if these devices were locally installed, we compare the performance of an \gls{io}-heavy workload running on a local system using a local device, to the same workload running on a remote machine using the same device borrowed through \gls{dl}.
%
...
%
This is an exploratory test, highlighting device driver functionality enabled by the \gls{sisciapiext}n on.
...
%
Standard and unmodified benchmarking software are used for our tests.
