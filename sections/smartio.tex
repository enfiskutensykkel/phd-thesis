...
%
Therefore, a device may only be distributed to a single \gls{borrowermachine} at the time in most cases, preventing others from using it while it is used.
%
...
%
As proof of this \gls{disaggregation} concept, we have implemented an \gls{nvme} driver as a \gls{userspace} \gls{sisci} application.
...
As illustrated in \cref{fig:nvme}, \glspl{nvme} support asynchronous operation by using a system of paired command \glspl{sq} and \glspl{cq}~\cite{spec:NVMe}.
%
...
%
``\Gls{db}'' on the \gls{nvme} device are used to signal when new commands should be fetched, and each queue has its own \gls{db} in order to avoid any contention in the command submission and completion paths.
%
Furthermore, as \emph{multiple} of these queue pairs can be created, parallel operation is supported.
%
An example of this is a multi-core \gls{cpu} that assigns an \gls{sq} and an associated \gls{cq} per \gls{cpu} core, where each core can operate the \gls{nvme} completely independent of each other.



%
\Cref{fig:nvme-driver} shows how our driver implementation utilize this, in order to enable multi-host sharing:
%
Each machine allocates a \gls{memorysegment} where it sets up the queue data structures, and uses the \gls{apiext} to map these segments for the device.
%
They also use the \gls{apiext} to memory map device registers through their respective \glspl{ntb}.

    {An \gls{nvme} can access queues residing in memory segments on different machines by mapping these segments for the device as \glspl{dmawindow}. \Glspl{borrower} can in turn map doorbell registers for their respective queues. The \gls{nvme} is able to fetch commands from queues in remote memory the same way it would fetch commands from queues in local memory on the \gls{lender}, using native \gls{dma} transfers. Since queues are completely parallel, each machine can submit commands independent of each other.}
