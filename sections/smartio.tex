...
Related \gls{disaggregation} solutions can be broadly divided into the following categories:
%
\begin{itemize}
    ...
    \item Distributed \gls{io} and sharing devices over a network using \textbf{\gls{rdma}-based device \gls{disaggregation}}. 
        %
        ...
        %
        As addressed in \cref{sec:motivation}, although \gls{rdma} enables efficient data transfer over a network through one-sided initiation and direct access to application memory, \gls{rdma} is not as closely integrated to the \gls{pcie} fabric as our own \gls{pcie}-based solution is;
        %
        \gls{rdma}-based approaches rely on communicating with a driver running on the device-side system, in order to interact with the remote device, and typically solve this by implementing a \gls{middlewareservice}.
        %
        As seen in \cref{fig:direct-access}, this additional software component inevitably leads to a performance overhead, compared to a local device driver accessing a device on the \gls{pcie} bus.
        %
        Furthermore, this limitation also means that \gls{rdma}-based solutions are specific to the device they \gls{disaggregate}.

        

        In comparison, our SmartIO framework enables access to remote resources over native \gls{pcie}.
        %
        Not only does this allow allow devices to remote devices to be used with near-local performance (\cref{sec:eval}), but our solution is more general, in the sense that it is possible to share \emph{any} \gls{pcie} device.
        %
        ...
        %
        Finally, it should also be pointed out that using SmartIO, \gls{rdma}-capable \glspl{nic} such as InfiniBand cards can be \lgls{borrower}{borrowed}, making it possible to run \gls{rdma} applications on top of SmartIO.


    \item Solutions that enable shared-memory communication through \textbf{memory \gls{disaggregation}}.
        marlin \cite{Tu2014}
        \cite{Hou2013}
        \cite{Lim2009}
