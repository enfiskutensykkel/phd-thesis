...
\footnote{We show how longer \gls{pcie} paths affects \gls{dma} performance in \paperref{cc:eval-native-p2p}.}
%
...
\begin{figure}
    ...
    {Histogram of the latency distributions for reads from storage for both the Local~Baseline and \gls{dl} scenarios. Each data point is the latency for a full 4~kB read operation. The distribution for both scenarios overlap, demonstrating that our implementation does not add any overhead in the critical \glsxtrshort{io} path.}
