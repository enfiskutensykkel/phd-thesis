...
\begin{figure}
    ...
    {The software architecture of SmartIO. Three different sharing methods are made possible by our framework: \textbf{(1)}~\gls{dl},\textbf{(2)}~\gls{mdev}, and \textbf{(3)}~using the \gls{sisciapiext}. The SmartIO driver, shown as \textbf{(B)}, abstracts away the physical location of remote resources, for both the physical device and software using the device.}
    \label{fig:architecture}
\end{figure}


In this section we give a bottom-up summary of the implementation of SmartIO.
%
\Cref{fig:architecture} illustrates the different components of SmartIO and how they fit together.
%
The full implementation details can be found in the published papers, particularly \cref{paper:tocs}.
...
%
For example, a \gls{borrower} can request that SmartIO maps a memory segment in the \gls{borrowermachine} so that the device may use \gls{dma} to it.
%
The SmartIO driver will set up the necessary maps and return a \emph{device-side} \gls{io} address that the \gls{borrower} can then pass to the device when initiating \gls{dma}.
%
The \gls{borrower} does not need to know anything about this address, other than the fact that SmartIO has resolved it so that it maps through the device-side, or \gls{lender}'s, \gls{ntb} in order for the device to reach the memory segment.
%
...
%
By relying on the SmartIO driver to resolve addresses, a \gls{borrower} may use remote \gls{io} addresses without requiring that it is aware of the internal \gls{io} address space layout of \glspl{lender}.
%
In other words, the SmartIO driver is the first step towards solving the challenges described in \cref{sec:challenges}.


More details on how SmartIO resolves the paths between devices and other memory resources that must be mapped for the device are given in the papers, particularly in \paperref{tocs:lending-p2p,cc:p2p}.
%
This also explains how memory regions of \emph{other} devices can be mapped for a device, even when they reside in a different \gls{lendermachine}.
...
%
Conversely, we can also map local resources, such as \gls{ram} and interrupt addresses, for the remote device, allowing it to use \gls{dma} through the \gls{ntb} and trigger interrupt routines on the \gls{cpu}.
%
By using the SmartIO driver, we eliminate the complexity of managing multiple (remote) address spaces and may rely on SmartIO to resolve where resources are located and configure maps through the appropriate \glspl{ntb}.
%
However, as pointed out in \cref{sec:challenges}, we still need a mechanism to make sure that device drivers use \gls{io} addresses that map through the \glspl{ntb} without requiring that they be re-written. 
%
In other words, a mechanism is needed for injecting resolved \gls{io} addresses without the driver or device being aware.

%making it appear to the OS

\begin{figure}
    \centering
    \includegraphics[width=.99\textwidth]{device-lending}
    \caption[Device~Lending injects a hot-added ``shadow device'' into the local \glsfmtshort{pcie} device tree, making remote device access transparent to both \glsfmtshort{cpu} and the device]
    {\gls{dl} inserts a \gls{shadowdev} into the local device tree, making it appear as if a device has been hot-added to the system. The \gls{shadowdev} makes it possible to intercept device interactions by a device driver and inject addresses that are mapped through the \gls{ntb} in a transparent manner.}
    \label{fig:device-lending}
\end{figure}

Our \emph{\gls{dl}} method, illustrated as arrow \textbf{(1)} in \cref{fig:architecture}, solves this.
