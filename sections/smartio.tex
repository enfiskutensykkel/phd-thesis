...
\Gls{disaggregation} and resource sharing solutions can broadly be divided into the following three categories:
%
\begin{itemize}
    \item Device sharing through \textbf{\Gls{pcie} switch partitioning}~\cite{whitepaper:Microsemi,Wong2011,whitepaper:IDT,Chung2018}.
        ...
        %
        As such, solutions based on switch partitioning can not support memory-to-memory communication between hosts, nor are they able to support memory \gls{disaggregation}.
        %
    ...
    \item Distributed \gls{io} and sharing devices over a network by \textbf{\gls{rdma}-based device \gls{disaggregation}}. 
        %
        ...
        %
        Solutions for \gls{rdma}-based \gls{disaggregation} over a \gls{pcie} network fabric have also been proposed for \glspl{gpu}~\cite{Hou2013} and \glspl{nic}~\cite{Tu2014}, by using \gls{pcie} switch chips capable of \gls{dma} to transfer data from memory to memory.
        

        As addressed in \cref{sec:motivation}, although \gls{rdma} enables efficient data transfer over a network through one-sided initiation and direct access to application memory, \gls{rdma}-based approaches rely on communicating with a driver running on the device-side system, in order to interact with the remote device.
        %
        ...
        %
        As such, \gls{rdma}-based solutions are specific to the (type of) device they \gls{disaggregate}.
        %
        Moreover, as seen in \cref{fig:direct-access}, this additional software component on the remote system inevitably leads to a performance overhead, compared to a local device driver directly accessing a device on the \gls{pcie} bus.
    ...
    \item Remote memory access and shared-memory communication and through \textbf{memory \gls{disaggregation}}.
        %
        This category 

        include both pcie backplane and network
        Some solutions have been proposed in the past for 

        Remote memory access through transparentl 
        \gls{rdma} to \gls{disaggregate} system memory~\cite{Liang2005,Gu2017}
        %
        Similar solutions have been suggested for blade servers connected to a shared \gls{pcie} backplane, by using \gls{pcie} switch chips capable of \gls{dma} instead~\cite{Lim2009}.
        %

        
        %

        More explicit control, \cite{Aguilera2018} and also mpi

        
        rdma has also been used
        application specific, distributed applications
        More commonly, perhaps, is implementing shared-memory communication based on \gls{mpi}~\cite{Huang2012,Jiang2004,Lu2013,Venkatesh2014,Venkatesh2017}.
        %

ntb based shared memory thing
        \cite{Lim2019,Shim2018}
...
There are also solutions that 
    %    suzuki, dredbox, shoal from nsdi19, 
...
ladon as a separate point here \cite{Tu2013,Tu2018}
