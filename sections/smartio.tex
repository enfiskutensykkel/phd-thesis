...
%
We are able to detect when a device driver is preparing \gls{dma} buffers through the \gls{shadowdev}, enabling us to inject the prepared device-side \gls{io} address of our \gls{dmawindow}.
%
...
Mapping a remote device to the same virtual address space as a poses a dual challenge.
%
First, to avoid making any assumptions about the \gls{vm} or the system it emulates, all of its \gls{guestphys} memory must be mapped.
%
In practice, \emph{any} (\gls{host}) memory allocated by a \gls{emulator} application can be potentially used to emulate \gls{guest} memory.
%
Additionally, the \gls{hostphys} memory pages may be scattered throughout physical \gls{ram}. 
%
We need a reliable method for detecting the memory allocated by a \gls{emulator} and determining which of those physical memory pages are used for backing \gls{guestphys} memory (and must be mapped for the device).
%
To further complicate matters, this method can not require any modifications to existing \gls{emulator} software, as per our objectives stated in \cref{sec:problem}.
%
Second, for normal \gls{passthrough}, the \gls{hypervisor} places the device in a \gls{iommu} domain with \gls{io} virtual addresses that correspond to the \gls{vm}'s address layout.
%
In our case, the device not only resides in \emph{different} machine,~i.e., the \gls{lender}, but the \gls{borrower}'s \gls{iommu} is needed to provide continuous address ranges to scattered physical memory, so this memory can be mapped through the \gls{lender}'s \gls{ntb} as continuous \glspl{dmawindow}.
