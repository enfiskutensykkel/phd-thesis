...
\begin{figure}
    \centering
    \includegraphics[width=.99\textwidth]{nvme-gpu-offloading}
    \caption
...
\begin{itemize}
    ...
    \item The improved \gls{dl} implementation with support for \gls{p2p} between devices is evaluated in \paperref{srmpds:eval-p2p}, where we perform \gls{p2p} experiments with \glspl{gpu} in several network topologies and test scenarios, demonstrating the performance benefit of \gls{p2p} \gls{dma} compared to bouncing data via \gls{ram}.
        %
        These tests are extended in \paperref{cc:eval-native-p2p} with additional network topologies and test scenarios.
        %
        We also show the performance a \gls{sriov}-capable \gls{nic} being shared with multiple machines at the same time in \paperref{tocs:eval-sriov}.



    \item Our \gls{mdev} implementation is evaluated in \paperref{srmpds:eval-mdev}, where the performance of a \gls{vm} using a \lgls{passthrough}{passed-through} is measured.
        %
        The impact of \glspl{iommu} on \gls{dma} transfers is demonstrated in \paperref{tocs:eval-iommu}.
        %
        In \paperref{tocs:eval-mdev-vfio}, we extend our \gls{mdev} evaluation and compare the performance of our \gls{mdev} implementation to ``regular'' \gls{passthrough} (of a local device) as well as bare-metal \gls{dl}.
        %
        ...
        %
        Additional \gls{p2p} experiments using \gls{mdev} are also presented in \paperref{cc:eval-mdev-p2p}.
        


    \item To prove that \gls{dl} and \gls{mdev} can be used for real-world applications, we evaluate the run-time performance of a machine~learning workload using borrowed \glspl{gpu} and a borrowed \gls{nvme} in \paperref{tocs:eval-ml,cc:eval-ml}.
        %
        To further demonstrate SmartIO being used for realistic applications, we present a \gls{gpu}-based video processing workload in \paperref{mmsys}, and use \gls{dl} to scale up the number of available \glspl{gpu} for the workload, in order to meet a real-time deadline.

    \item Several experiments using our our proof-of-concept \gls{nvme} driver are presented in \paperref{tocs:eval-nvme}, demonstrating capabilities made possible through the \gls{sisciapiext}.
        %
        We show how 30~cluster~nodes can share a single-function \gls{nvme} simultaneously, how data stored on the \gls{nvme} can be \lgls{multicasting}{multicasted} and replicated across 60~nodes in a single operation, as well as interoperability with both local and remote \glspl{gpu}.
        %
        A comparison of our proof-of-concept \gls{nvme} driver and a state of the art \gls{nvmeof} solution is presented in \paperref{tocs:eval-nvmeof}, showing that our \gls{nvme} driver outperforms \gls{rdma}.


    \item A complete performance evaluation of SmartIO in its entirety is presented in \paperref{tocs:eval}. 
        %
...
In this section, we present shortened versions of three selected performance tests from \cref{tocs}.
%
...
\end{figure}
%
\Cref{fig:eval-lending-nvme-topo} shows the hardware configurations for the two test scenarios:
\begin{description}
...
We configured \gls{fio} to perform 655360 reads, where each read is a page-sized block~(4~kB) at a random offset on disk, also known as a ``random read'' pattern.
%
...
\begin{figure}
    ...
    \par\vspace{5mm}
    \caption
    [Hardware configuration for the two scenarios in our \glsfmtshort{dma} throughput comparison experiment. The expansion chassis adds the same number of ``hops'' between the \glsfmtshort{cpu} and the \glsfmtshort{gpu} in both scenarios]
    {Hardware configuration for the two scenarios in our \glsxtrshort{dma} throughput comparison experiment. The expansion chassis adds the same number of ``hops'' between the \glsxtrshort{cpu} and the \glsxtrshort{gpu} in both scenarios.}
    \label{fig:eval-lending-gpu-topo}
\end{figure}


Another performance metric is throughput, particularly the throughput for transferring from a device to \gls{ram}.
%
...
\Cref{fig:eval-lending-gpu-topo} illustrates the hardware configuration used in our throughput test scenarios:
\begin{description}
...
\begin{figure}
    ...
    \caption
    [The median \glsfmtshort{dma} write throughput of different transfer sizes for the Local Baseline and Device Lending scenarios. The measured performance is the same for both scenarios, demonstrating that our implementation does not add any overhead]
    {The median \glsxtrshort{dma} write throughput of different transfer sizes for the Local Baseline and \gls{dl} scenarios. The measured performance is the same for both scenarios, demonstrating that our implementation does not add any overhead.}
    \label{fig:eval-lending-gpu-results}
\end{figure}


\Cref{fig:eval-lending-gpu-results} depicts the results of our test.
%
...
%
This experiment is described in further detail in \paperref{tocs:eval-lending-bw}.
...
%
Since most device drivers are written in a way that assumes exclusive control of a device, devices shared with these methods may (in most cases) only be used by a single \gls{borrower} at the time.
%
For distributed cluster applications, this poses a challenge.
%
For example, if a workload needs to access data on a storage device or perform computation on a \gls{gpu}, the application process must interact with the device driver.
%
...
\begin{figure}
    ...
    \begin{subfigure}{\linewidth}
        \centering
        \includegraphics[width=.99\textwidth]{sq-experiments-buffers}
        \caption%
        {Our proof-of-concept \glsfmtshort{nvme} driver is running on the borrower's \glsxtrshort{cpu}, operating a remote \glsxtrshort{nvme}. The remote \glsxtrshort{gpu} is borrowed using \gls{dl}, and both \glsxtrshortpl{gpu} are operated using the native \gls{cuda} driver. Data is loaded from storage directly into \glsxtrshort{gpu} memory using a random read access pattern.}
    ...
    \begin{subfigure}{\linewidth}
        \centering
        \includegraphics[width=.99\textwidth]{sq-experiments-queues}
        \caption%
    ...
    \end{subfigure}
    %
    \caption[The design of our proof-of-concept \glsfmtshort{nvme} driver experiment]
    {The design of our proof-of-concept \glsxtrshort{nvme} driver experiment.}
    \label{fig:eval-nvme-design}
\end{figure}
%
It is possible to remedy this problem by using our \gls{exttosisciapi} to develop a device driver as part of the distributed application itself.
%
...
%
To demonstrate this, we have designed an experiment using the following workload (as illustrated in \cref{fig:eval-nvme-design}):
%
\begin{itemize}
    \item 
        %
        An application process is running on the \gls{borrowermachine}, using both the local Nvidia~P600 \gls{gpu} and the remote Nvidia~P4000 \gls{gpu}, as well as a remote Intel~Optane~P4800X~DC \gls{nvme}.
        %
        The remote \gls{gpu} is borrowed using \gls{dl}, while the remote \gls{nvme} is operated by our proof-of-concept \gls{nvme} driver, and is implemented as part of the \gls{userspace} application process.
        %
        The two \glspl{gpu} are managed by the native \gls{gpu} drivers (\gls{cuda} version~10.2).
    ...
    \item 
        ...
        %
        \begin{description}
            \item[Borrower~\gls{ram}] the \gls{sq} is allocated in local \gls{memorysegment} on the same machine where our application process is running.
            \item[Lender~\gls{ram}] the \gls{sq} is allocated in \gls{lender}'s \gls{ram}, by allowing SmartIO to choose based on access pattern hinting.
            \item[Borrowed~\gls{gpu}] the \gls{sq} is allocated in memory on the borrowed \gls{gpu}.
        \end{description}
        %
    ...
    \item 
        ...
        %
        For each \gls{gpu}, we performed 327,680 reads (655,360 in total).
        %
        ...
        %
        However, both \glspl{gpu} are included as destinations in our experiment, for the purpose of showcasing the capabilities of SmartIO.
...
\begin{figure}
    ...
    \caption{Distribution of recorded command completion latencies as a histogram (left) and as a boxplot (right). The closer the \glsfmtshort{sq} is to the \glsfmtshort{nvme}, the lower the latency is.}
    \label{fig:eval-nvme-results}
\end{figure}
%
\Cref{fig:eval-nvme-results} depicts the distributions of recorded latency values for \gls{io} read operations, for all three scenarios.
%
The same datasets are show as a histogram (on the left) and as a boxplot (on the right).
%
Note that we have adjusted the Y-axis, so suspected outlier measurements are not shown.
%
Also note that the median for all three scenarios are marked with horizontal lines.
%
The results demonstrate that by moving \gls{sq} memory closer to the \gls{nvme} reduces the latency, because the distance the \gls{nvme} has read across shrinks (see \cref{fig:eval-nvme-scenarios} for reference):
%
\begin{itemize}
    \item For the \textbf{Borrower~\gls{ram}} scenario, the \gls{nvme} has to read across the internal switch chip in the expansion chassis, the lender's \gls{ntb} adapter, the cluster switch, and the borrower's \gls{ntb} adapter (4~``hops'').
        %
    \item In the \textbf{Lender~\gls{ram}} scenario, the \gls{nvme} reads across the internal switch chip, and both the downstream and upstream transparent switches, in order to fetch commands (3~``hops'').
        %
    \item By using the \gls{gpu} installed alongside the \gls{nvme} in the same expansion chassis in the \textbf{Borrwed~\gls{gpu}} scenario, the \gls{nvme} only has to read through the internal switch chip in order to fetch commands (1~``hop'').
\end{itemize}




While there is an associated development cost of implementing device drivers using the SmartIO \gls{apiext}, we argue that this experiment proves that there is a clear reward in the improved performance over using native device drivers.
%
There is also the added benefit of being able to fully utilize \gls{pcie} shared-memory capabilities, like streaming data directly into \gls{gpu} memory as shown in this experiment.
%
Furthermore, by integrating driver operation with the \gls{sisciapi}, additional capabilities also become available, such using \gls{pcie} \gls{multicasting} to replicate data across several cluster machines in a single operation.



A similar experiment is presented in \paperref{tocs:eval-nvme-sq}.
