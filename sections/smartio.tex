...
%
``\Gls{db}'' on the \gls{nvme} device are used to signal when new commands should be fetched, and each queue has its own \gls{db}.
%
...
%
Since queues are completely parallel, each machine can submit \gls{io} commands and receive completions, entirely independent of each other.
%
Note that all machines are able to operate the \gls{nvme} at the same time, including the \gls{lender}.
%
The software is the same regardless of which machine it runs on, since SmartIO keeps track of where the \glspl{segment} and the device reside, and is able to decide where \glspl{segment} should be allocated based on memory access pattern hinting, as explained in \cref{sec:api}.
