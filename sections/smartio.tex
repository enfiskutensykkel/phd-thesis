...
\begin{description}
    ...
    \item[Initiating \gls{io} from the \gls{gpu}] (\cref{fig:nvme-gpu-offloading}):
\begin{figure}
    \centering
...
\end{figure}
%
        The logic for submitting \gls{io} commands to an \gls{sq} and polling an \gls{cq} for completions is also implemented for \gls{cuda} applications in our \gls{nvme} driver.
        %
        Similarly to how \gls{gpu} memory can be mapped for an \gls{nvme} using SmartIO, it is also possible to \textbf{map \glspl{db} for the \gls{gpu}}.
        %
        This makes it possible to run the \gls{nvme} driver \emph{on a \gls{gpu}}, and the \gls{nvme} can be controlled directly from software running on the \gls{gpu} without any involvement from the \gls{cpu}.
        %
        As \gls{gpu} threads can read from and write to storage directly, \gls{cuda} applications that need to load and store data at various points in the computation no longer need to synchronize with the \gls{cpu}.
...
%
Shim~et~al.~\cite{Shim2018,Lim2019} present an extension of the OpenSHMEM \gls{api} for the \gls{pgas}-style of shared memory, using their \gls{ntb} implementation.
%
By using the same Broadcom~8733~\cite{pex8733} \gls{pcie} switch chips used in the Dolphin~PXH830 adapter cards used in our evaluation (\cref{sec:eval}), they have implemented \gls{ntb} host adapter cards 

They implemented \gls{ntb} adapter cards and connecting three machines in a \gls{pcie} cluster

Shim~et~al.~\cite{Shim2018,Lim2019} 
An interesting solution has been proposed by 
have developed a solution for extending a \gls{pgas}

using \gls{pcie} switch chips to develop \gls{ntb} adapter cards and connected three machines in a cluster.
%
By extending a shared-memory \gls{api} with support for their \gls{ntb} implementation, they have implemented \gls{pgas} that is very similar to how the \gls{sisciapi} works.
%
