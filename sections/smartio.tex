...
\begin{itemize}
    \item Device sharing through so-called ``\textbf{rack-scale \gls{disaggregation}}'', where modular blade servers are connected to a backplane, or some other shared \gls{io} bus fabric, within a data center rack.
        ...
        %
        We also include \gls{mriov}~\cite{spec:MRIOV} in this category, even though we are unaware if there are any available implementations of it.

        
        \Gls{pcie}-based solutions for rack-scale \gls{disaggregation} use \gls{pcie} switch chips with virtualization support for \emph{partitioning} the \gls{pcie} fabric~\cite{Wong2011,whitepaper:Microsemi,whitepaper:IDT,Chung2018}.
        %
        As illustrated in \cref{fig:partitioning}, this partitioning makes it possible to isolate devices and \glspl{cpu} by creating logical device trees for each host.
        %
...
%        Finally, it should also be pointed out that \gls{rdma}-capable \glspl{nic}, such as InfiniBand cards, can be \lgls{borrower}{borrowed} using SmartIO, making it possible to run \gls{rdma} applications on top of SmartIO.
    ...
    \item \textbf{Memory \gls{disaggregation}} through remote memory access.
        %
        This category includes solutions that require the use of parallel programming models for accessing remote memory, such as \gls{mpi}~\cite{Jiang2004}, \gls{rpc}~\cite{Lu2013}, or through other abstractions~\cite{Aguilera2018}.
        %
        ...
        Memory \gls{disaggregation} solutions, in particular those that use explicit programming models, are not so much aimed at sharing \gls{io} resources, but rather enabling \emph{shared-memory communication} for distributed cluster applications.
        %
        Especially \gls{mpi} protocols implemented using \gls{rdma} appears to be widely adopted for this purpose.
        %
        Many of these solutions also support \gls{gpudirect}, making it possible to \gls{disaggregate} \gls{gpu} memory~\cite{Venkatesh2014,Venkatesh2017,url:Rosetti2014}.
        %
        However, as with using \gls{rdma} for sharing devices over a network, \gls{rdma} is an additional layer of indirection for memory \gls{disaggregation} solutions too;
        %
        compared to a \gls{cpu} (or device) reading and writing to a memory location directly, initiating \gls{rdma} transfers can significantly increase latency.
