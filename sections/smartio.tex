...
By integrating shared-memory functionality with device operation, we enable the implementation of device drivers that fully utilize the capabilities of \gls{pcie} networks.
%
For instance, applications may use \gls{pciemulticasting} to stream data to several destinations in a single operation, or exploit memory locality in order to optimize data flow through the network.
%
It is possible to combine the \gls{apiext} with the other sharing methods of SmartIO, allowing even \gls{disaggregation} \emph{borrowed} resources.
%
An example of this is would be a machine using \gls{dl} to borrow a remote \gls{gpu} capable of GPUDirect~\cite{url:GPUDirect,url:Rosetti2014}, allowing the device to be operated by the native driver, and then using the \gls{apiext} to export \gls{gpu} memory as a \gls{sharedsegment}.
%
This would allow other machines and devices to read and write directly to the \lgls{borrower}{borrowed} \gls{gpu}'s memory.



The possibilities created by the \gls{apiext} are perhaps best demonstrated by our proof-of-concept \gls{nvme} driver, described in \cref{sec:nvme-driver}.
%
This driver shows how a non-\gls{sriov} \gls{nvme} can be \gls{disaggregated} in software and shared by several machines.
%
