...
%
\begin{itemize}
    \item So-called ``rack-scale \gls{disaggregation}'' by \textbf{device assignment through logical partitioning}, where modular blade servers are connected to a backplane or a shared \gls{io} bus fabric within a data center rack.
        %
    ...
    \item Memory \gls{disaggregation} through \textbf{remote memory access}.
        %
        The goal of memory \gls{disaggregation} solutions is not so much sharing \gls{io} resources, but to facilitate shared-memory communication for distributed cluster applications, most commonly by using \gls{rdma}.
        %
        This category encompasses implementations of parallel programming models, such as \gls{mpi}~\cite{Jiang2004} and \gls{rpc}~\cite{Lu2013}, as well as other abstractions for remote memory access~\cite{Aguilera2018}.
        %
        Approaches for accessing remote memory in a more \emph{transparent} fashion also exist, usually achieved through ``\lgls{trap}{faulting in}'' remote memory pages by modifying the system page fault handler to initiate \gls{rdma} transfers~\cite{Liang2005,Gu2017,Lim2009}.
...
\begin{itemize}
    ...
    \item In contrast to \gls{rdma}-based solutions for memory \gls{disaggregation}, remote \glspl{memorysegment} can be mapped into a local software application's virtual address space directly using the \gls{sisciapi}.
        ...
        %
        The need for \gls{rdma} for shared-memory communication is removed entirely, as both \glspl{cpu} and devices may instead read and write to remote memory directly.
        %
...
\end{itemize}
%
We present a more technically detailed comparison between specific solutions and our own SmartIO implementation in the published papers, particularly in \paperref{tocs:rw-partitioning,tocs:rw-rdma,tocs:rw-disaggr}.
%
In \paperref{tocs:rw-nvme}, we also present related background for our proof-of-concept \gls{nvme} driver implementation.
