...
%
Similarly, \glspl{memorysegment} can be mapped for a device, allowing devices may use native \gls{dma} to access \gls{sharedsegment} directly.
%
...
%
Even so, being able to integrate device drivers into the cluster application itself may potentially be extremely useful for some application domains.
%
...
%
Therefore, a device may, in most cases, only be distributed to a single \gls{borrowermachine} at the time, preventing others from using it while it is used.
%
...
As mentioned in \cref{sec:api}, the \gls{sisciapiext} makes it possible for several machines to share the same device (\gls{function}), by facilitating \gls{disaggregation} in software instead.
%
In order to demonstrate this, we have developed a proof-of-concept \gls{nvme} device driver using our \gls{apiext}.


Something about NVMe being particularly well-suited because parallel
Mention queues and DMA, not too many details
Figure should show four machines, but with one machine hosting queues in GPU memory
Figure should also say that it is possible to map nvme device registers for gpu
