...
%
\Gls{passthrough} is made possible by using the \gls{iommu} to create a virtual \gls{io} address space for a device that corresponds to the virtual address space of the \gls{vm}~\cite{url:LinuxVFIO,url:LinuxIOMMU,Muli2006}.
%
...
%
A device driver running in the \gls{vmguest} is completely isolated, and may try to use any (virtualized) memory address when initiating \gls{dma} transfers.
%
Consequently, supporting \gls{passthrough} requires devising a method for mapping the \gls{hostphys} memory of the \gls{emulator} through the \gls{ntb} that also mirrors the virtual address space of the \gls{vm}.
...
%
Unlike \gls{dl}, however, we have no way of knowing which addresses a device driver in the \gls{guest} may attempt to use for \gls{dma}.
%
In \gls{dl}, we prepare an \gls{iommu} range in advance, and map it through the \gls{lender}'s \gls{ntb} in advance. 
%
The \gls{shadowdev} makes it possible to detect when a device driver is preparing \gls{dma} buffers, and we can inject the device-side \gls{io} address of our \gls{dmawindow}.
%
In contrast, a device driver running in the \gls{vmguest} is completely isolated, leaving us without any equivalent mechanism to inject \gls{io} addresses we have prepared.
%
As such, the only possible option is to make sure that the device is mapped to the same virtual address space as the \gls{vm}.



As pointed out in \cref{sec:challenges}, this requires that all of the memory used by the \gls{emulator} application is mapped.
%
s

left without a reliable mechanism to detect when dma transfers are being initiated



through the \gls{lender}'s \gls{ntb}.
%


In addition, and to map 


We are not able to know which memory addresses the driver in the \gls{guest} is using
%
To make 
%
%
...
create a virtual \gls{io} address space that mirrors the \gls{guestphys} memory layout of the \gls{vm}.
...
%Men:
%- assumes local device, will not work on the other side of ntb
%- need to detect emulator memory, without requiring modifications to qemu

%As the driver running in the \gls{vmguest} is completely isolated,  will not only be unaware of the fact that the device is on the other side of an \gls{ntb}, but also that it is running in a virtualized address space.
%



%
%the only solution is to map the entire memory of the \gls{vm} and use \glspl{iommu} to ensure that the device is mapped to the same virtual address space as seen by the \gls{vmguest}.
%%
%However, this requires a mechanism to detect \gls{hostphys} memory used by the \gls{emulator}, so that it may be mapped through the \gls{ntb}.
%
%as well as a method of discovering the virtual address space so that it may be mirrored for the device.
...
as the memory allocated by a \gls{emulator} can not be known in advance.

and use the \gls{lender}'s \gls{iommu} to create a virtual \gls{io} address space that mirrors 
%
