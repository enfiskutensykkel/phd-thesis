...
%
In this section, we attempt to provide a condensed overview of related work we consider the most relevant.
%
More technical comparisons between specific solutions and our SmartIO solution are provided in \paperref{tocs:rw}.


\subsection{Disaggregation solutions}
State of the art within \gls{disaggregation} and resource sharing can broadly be divided into three categories:
%
\begin{itemize}
    \item So-called ``rack-scale \gls{disaggregation}'' and device sharing by \textbf{logical partitioning}, where modular blade servers are connected to a backplane or a shared \gls{io} bus fabric within a data center rack.
        %
        ...
        %
        \gls{pcie} (with additional virtualization support) is by far the most often used alternative for commercial solutions~\cite{url:liqid,url:gigaio,Chung2018,Ravindran2008}.
        %
        ...
        %
        However, as we addressed in \cref{sec:motivation}, it is only possible to distribute resources that are directly attached to these partitionable switch chips, and it is not possible to share the \emph{internal resources} of a machine (such as its memory and local devices).
        %
        ...
        %
        Instead, additional \gls{disaggregation} solutions are needed in order to support this, for instance solutions that use \gls{rdma}.


    \item Memory \gls{disaggregation} through \textbf{remote memory access}.
        %
        The goal of memory \gls{disaggregation} solutions is not so much sharing \gls{io} resources, but rather to use \gls{rdma} to facilitate shared-memory communication for distributed cluster applications.
        %
        Therefore, this category encompasses implementations of parallel programming models, such as \gls{mpi}~\cite{Jiang2004} and \gls{rpc}~\cite{Lu2013}, as well as other abstractions~\cite{Aguilera2018}.
        %
        Approaches for accessing remote memory in a more \emph{transparent} fashion also exist, most commonly achieved through ``\lgls{trap}{faulting in}'' remote memory pages by modifying the system page fault handler to initiate \gls{rdma} transfers~\cite{Liang2005,Gu2017,Lim2009}.
        

        Especially \gls{mpi} protocols implemented using \gls{rdma} over InfiniBand appear to be widely used for distributed computing.
        %
        ...
        %
        Although \gls{rdma} enables efficient data transfer over a network through one-sided initiation and direct access to application memory, it nonetheless introduces a layer of indirection;
        %
        compared to a \gls{cpu} (or device) reading and writing to a memory location directly, latency is significantly increased by needing to initiate (and wait for) \gls{rdma} transfers.
    ...
    \item Device \gls{disaggregation} by \textbf{distributed \gls{io} over a network} using \gls{rdma}.
        ...
        \Gls{rdma}-based approaches rely on communicating with a driver running on the device-side system, in order to interact with the remote device.
        %
        ...
        %
        Because of this, \gls{rdma}-based device \gls{disaggregation} solutions are specific to the (type of) device they \gls{disaggregate}, as mentioned in \cref{sec:motivation}.
        %
...
\begin{itemize}
    ...
    \item In contrast to \gls{rdma}-based solutions for memory \gls{disaggregation}, remote \glspl{memorysegment} can be mapped into a local software application's virtual address space directly.

    \item devices can use native dma, no rdma is needed

        are accessed over native \gls{pcie}.
        %
        No software or \gls{rdma} initiation is needed in the performance critical path, as the \gls{sisciapi} makes it possible to memory map remote \glspl{memorysegment} into a local software application's virtual address space.
...
%        Not only does this allow devices to read and write to remote memory using native \gls{dma}, but we are even able to make device operation part of the cluster application itself.
    \item underneath the os, smartio is general


\end{itemize}
%
By having the flexibility of shared-memory solutions \emph{and} the performance advantage of native \gls{pcie}, SmartIO is ``the best of both worlds''.
%
%

%%
%In comparison to \gls{pcie}-based rack-scale solutions that require resources to be attached directly to switch chips in order to be shared, SmartIO avoids this kind of central component by being fully distributed, allowing all machines to lend out their internal devices and use the internal resources of other machines.
%%
%The inherent memory mapping capabilities of \glspl{ntb} make it possible to 
%
%implement shared-memory communication, allowing SmartIO to support sharing on several levels.
%
...
%By using \glspl{ntb}, our SmartIO solution is 
%
