...
\newglossaryentry{function}{
    ...
    text={function},
    description={\shortgls{pcie} devices may have one or more functions, each appearing to the system as individual devices with their own set of resources. Also known as a \shortgls{pcie} endpoint},
    seealso={gls-sriov},
}
...
{Using the network adapter to copy memory directly onto the network, without going through the \longgls{os} network stack, often used to implement middleware~services}
...
{In \shortgls{pcie}, device memory, such as registers, are mapped to the same address space as \shortgls{ram}, allowing the \shortgls{cpu} to read and write to this memory the same way it would access system~memory}

\newglossaryentry{middleware}{
    name={middleware},
    description={A software service that provides facilitation beyond functionality available from the \longgls{os}, often implemented using \shortgls{rdma}}
}
