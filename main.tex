...
% Refer to a section in a paper from the main body of the dissertation
% Also provides a page number reference
\NewDocumentCommand{\papersectref}{ m m }{%
    \ifthenelse{\equal{#2}{}}{%
        \hyperref[#1]{\cref*{#1}}~(on~\cpageref{#1})%
    }{%
        \hyperref[#1:#2]{\cref*{#1:#2} of \cref*{#1}}~(on~\cpageref{#1:#2})%
    }%
}

% Convenience macro for referring to a specific section of a ppaper
